  ...
  <td colspan="2">
    <div class="sd-div" id="shipping_fee_content">
      ...
      <div id="myModal" class="sd-modal">
        ...
        <div class="sd-modal-content" style="text-align:center">
          <p style="text-align: left;">
            <?php _e('Vui lòng chọn 1 trong những địa điểm dưới đây để tạo vận đơn Giao hàng tiết kiệm.', 'ship-depot-translate') ?>
          </p>
      ...
      <div id="courier_<?php echo esc_attr($courier->CourierID) ?>" class="courier-fee">
        <img src="<?php echo esc_url($courier->LogoURL) ?>" alt="<?php echo esc_attr($courier->CourierName) ?>"
          data-placement="bottom" title="<?php echo esc_attr($courier->CourierName) ?>">

        <input type="hidden" name="shipdepot_courier_data_<?php echo esc_attr($courier->CourierID) ?>"
          value="<?php echo esc_attr(Ship_Depot_Helper::ParseObjectToJsonHTML($courier_clone)) ?>" />
        <div>
          ...
          <div class="service-fee">
            <input type="hidden" id="shipping_<?php echo esc_attr($serv->ServiceCode) ?>"
              name="shipdepot_shipping_data_<?php echo esc_attr($serv->ServiceCode) ?>"
              value="<?php echo esc_attr($json_html) ?>">
            <input type="radio" id="rd_<?php echo esc_attr($serv->ServiceCode) ?>" name="shipdepot_shipping_selected"
              class="radio_shipping_fee" value="<?php echo esc_attr($serv->ServiceCode) ?>"
            ...
            <div class="service-fee-info">
              ...
              <?php
                                            ...
                                            if ($courier->CODFailed->IsUsed && $courier->CODFailed->ContentCheckout->IsShow && !Ship_Depot_Helper::check_null_or_empty($courier->CODFailed->ContentCheckout->Content)) {
                                                ?>
              <p class="cod-failed-description">
                <?php echo esc_html(trim($courier->CODFailed->ContentCheckout->Content) . ' ' . Ship_Depot_Helper::currency_format($courier->CODFailed->CODFailedAmount)) ?>
              </p>
              <?php
                                            }
                                            ?>
      ...
      <div style="margin: 10px 0 20px 0; display: flex; flex-wrap: wrap;align-items: center;">
        <p style="color:#0064ff;margin-right: 10px;">
          <?php _e('Nếu đã điền đủ thông tin thanh toán/vận chuyển mà không thấy danh sách đơn vị vận chuyển thì vui lòng bấm vào đây', 'ship-depot-translate') ?>
        </p>
      ...
      <?php
                ...
                ?>
      <input type="hidden" id="SDOrderTotal"
        value="<?php echo esc_attr($cart_total_without_shipping + GetShippingFeeSession()) ?>" />
      ...
      jQuery(document).ready(function($) {
        ...
        amountEle.html(formatVNCurrency(
          <?php echo esc_attr($cart_total_without_shipping + GetShippingFeeSession()) ?>));
      });
