...
<div class="dokan-withdraw-status-filter-container">
  <ul class="list-inline subsubsub" style="display: inline-block;">
    <li<?php echo $current === 'pending' ? ' class="active"' : ''; ?>>
      <a
        href="<?php echo esc_url( dokan_get_navigation_url( 'withdraw-requests' ) ); ?>"><?php esc_html_e( 'Yêu cầu đang đợi giải quyết', 'dokan-lite' ); ?></a>
      </li>
      <li<?php echo $current === 'approved' ? ' class="active"' : ''; ?>>
        <a
          href="<?php echo esc_url( $approve_menu_url ); ?>"><?php esc_html_e( 'Yêu cầu được phê duyệt', 'dokan-lite' ); ?></a>
        </li>
        <li<?php echo $current === 'cancelled' ? ' class="active"' : ''; ?>>
          <a
            href="<?php echo esc_url( $cancelled_menu_url ); ?>"><?php esc_html_e( 'Yêu cầu đã hủy', 'dokan-lite' ); ?></a>
          </li>
  </ul>
  <span class="dokan-add-product-link" style="float: right; display: inline-block;">
    <?php if ( dokan_withdraw_is_manual_request_enabled() ) : ?>
    <button class="dokan-btn" id="dokan-request-withdraw-button"><i class="fa fa-money">&nbsp;</i>
      <?php esc_html_e( 'Yêu cầu rút tiền', 'dokan-lite' ); ?> </button>
    <?php endif; ?>
    <a href="<?php echo esc_url( dokan_get_navigation_url( 'withdraw' ) ); ?>"
      class="dokan-btn"><?php esc_html_e( ' Bảng điều khiển rút tiền', 'dokan-lite' ); ?> </a>
  </span>
</div>