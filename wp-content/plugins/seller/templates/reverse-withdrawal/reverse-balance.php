...
<div class="reverse-balance-section">
  <div class="reverse-balance">
    <?php
        esc_html_e( 'Số dư thanh toán ngược: ', 'dokan-lite' );
        if ( $balance < 0 ) {
  ...
  <div class="reverse-pay-form">
    <input type="text" id="reverse_pay_balance"
      value="<?php echo esc_attr( wc_format_localized_price( wc_format_decimal( $payable_amount, wc_get_price_decimals() ) ) ); ?>"
      readonly="readonly" />
    <input type="button" id="reverse_pay" class="button dokan-btn dokan-btn-success dokan-btn-lg dokan-theme"
      value="<?php esc_attr_e( 'Thanh toán ngay', 'dokan-lite' ); ?>" />
  </div>
  <?php endif; ?>
</div>