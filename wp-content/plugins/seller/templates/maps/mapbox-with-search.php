...
<div class="dokan-map-wrap">
  <div class="dokan-map-search-bar">
    <input id="dokan-map-add" type="hidden" class="dokan-map-search" value="<?php echo esc_attr( $map_address ); ?>"
      name="find_address" placeholder="<?php esc_attr_e( 'Address', 'dokan-lite' ); ?>" size="30" />
    <a href="#" class="dokan-map-find-btn" id="dokan-location-find-btn"
      type="button"><?php esc_html_e( 'Tìm địa chỉ', 'dokan-lite' ); ?></a>
  </div>
...
jQuery(document).ready(function($) {
  ...
  function onMarkerDragEnd() {
    ...
    var url = urlOrigin + '/geocoding/v5/mapbox.places/' + lng + '%2C' + lat + '.json?access_token=' + accessToken +
      '&cachebuster=' + +new Date() + '&autocomplete=true';
  ...
  dokanMapbox.on('load', function() {
    dokanGeocoder = new MapboxGeocoder({
      ...
      placeholder: '<?php esc_html_e( 'Tìm kiếm địa chỉ', 'dokan-lite' ); ?>',
      marker: false,
      reverseGeocode: true,
    });
