...
<tr class="fee <?php echo ! empty( $class ) ? esc_attr( $class ) : ''; ?>"
  ...
  <td class="name">
    <input type="text" placeholder="<?php esc_attr_e( 'Fee Name', 'dokan-lite' ); ?>"
  ...
  <td class="line_cost" style="width: 1%;">
    <label>
      <?php esc_html_e( 'Tổng cộng', 'dokan-lite' ); ?>:
      <input type="text" name="line_total[<?php echo esc_attr( absint( $item_id ) ); ?>]" placeholder="0.00"
        value="<?php echo isset( $item['line_total'] ) ? esc_attr( $item['line_total'] ) : ''; ?>" class="line_total" />
    </label>
  </td>

  <?php if ( get_option( 'woocommerce_calc_taxes' ) === 'yes' ) : ?>
  <td class="line_tax" style="width: 1%;">
    <input type="text" name="line_tax[<?php echo esc_attr( absint( $item_id ) ); ?>]" placeholder="0.00"
