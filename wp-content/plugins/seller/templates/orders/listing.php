...
<form id="order-filter" method="POST" class="dokan-form-inline">
  ...
  <div class="dokan-form-group">
    <label for="bulk-order-action-selector"
      class="screen-reader-text"><?php esc_html_e( 'Chọn hành động hàng loạt', 'dokan-lite' ); ?></label>
    ...
    <select name="status" id="bulk-order-action-selector" class="dokan-form-control chosen">
      ...
      <option class="bulk-order-status" value="<?php echo esc_attr( $key ); ?>"><?php echo esc_attr( $value ); ?>
      </option>
      <?php } ?>
    </select>
  </div>
  ...
  <div class="dokan-form-group">
    ...
    <input type="submit" name="bulk_order_status_change" id="bulk-order-action" class="dokan-btn dokan-btn-theme"
      value="<?php esc_attr_e( 'Lưu', 'dokan-lite' ); ?>">
  </div>
  <?php } ?>
  <table class="dokan-table dokan-table-striped">
    <thead>
      <tr>
        ...
        <th><?php esc_html_e( 'Đơn hàng', 'dokan-lite' ); ?></th>
        <th><?php esc_html_e( 'Tổng số đơn hàng', 'dokan-lite' ); ?></th>
        <th><?php esc_html_e( 'Thu nhập', 'dokan-lite' ); ?></th>
        <th><?php esc_html_e( 'Trạng thái', 'dokan-lite' ); ?></th>
        <th><?php esc_html_e( 'Khách hàng', 'dokan-lite' ); ?></th>
        <th><?php esc_html_e( 'Ngày', 'dokan-lite' ); ?></th>
        <?php if ( function_exists( 'dokan_get_order_shipment_current_status' ) && 'on' === $allow_shipment && $wc_shipping_enabled ) : ?>
        <th><?php esc_html_e( 'Lô hàng', 'dokan-lite' ); ?></th>
        <?php endif; ?>
        <?php do_action( 'dokan_order_listing_header_before_action_column' ); ?>
        <?php if ( current_user_can( 'dokan_manage_order' ) ) { ?>
        <th width="17%"><?php esc_html_e( 'Hoạt động', 'dokan-lite' ); ?></th>
        <?php } ?>
      </tr>
    </thead>
    <tbody>
...
<div class="dokan-error">
  <?php esc_html_e( 'Không tìm thấy đơn hàng nào', 'dokan-lite' ); ?>
</div>
