...
<form id="dokan-vendor-register" method="post" class="register dokan-vendor-register">
  ...
  <div class="split-row name-field form-row-wide">
    <p class="form-row form-group">
      <label for="first-name"><?php esc_html_e( 'Tên của bạn', 'dokan-lite' ); ?> <span
          class="required">*</span></label>
      <input type="text" class="input-text form-control" name="fname" id="first-name"
        value="<?php echo ! empty( $data['fname'] ) ? esc_attr( $data['fname'] ) : ''; ?>" required="required" />
    </p>

    <p class="form-row form-group">
      <label for="last-name"><?php esc_html_e( 'Họ của bạn', 'dokan-lite' ); ?> <span class="required">*</span></label>
      <input type="text" class="input-text form-control" name="lname" id="last-name"
        value="<?php echo ! empty( $data['lname'] ) ? esc_attr( $data['lname'] ) : ''; ?>" required="required" />
    </p>
  </div>

  <?php if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) ) : ?>
  <p class="form-row form-group form-row-wide">
    <label for="reg_username"><?php esc_html_e( 'Tên đăng nhập', 'dokan-lite' ); ?> <span
        class="required">*</span></label>
    <input type="text" class="input-text form-control" name="username" id="reg_username"
      value="<?php echo ! empty( $data['username'] ) ? esc_attr( $data['username'] ) : ''; ?>" required="required" />
  </p>
  <?php endif; ?>

  <p class="form-row form-group form-row-wide">
    <label for="reg_email"><?php esc_html_e( 'Địa chỉ email', 'dokan-lite' ); ?> <span class="required">*</span></label>
    <input type="email" class="input-text form-control" name="email" id="reg_email"
      value="<?php echo ! empty( $data['email'] ) ? esc_attr( $data['email'] ) : ''; ?>" required="required" />
  ...
  <p class="form-row form-group form-row-wide">
    <label for="shop-phone"><?php esc_html_e( 'Số điện thoại', 'dokan-lite' ); ?><span class="required">*</span></label>
  ...
  <?php if ( get_option( 'woocommerce_registration_generate_password', 'no' ) !== 'yes' ) : ?>
  <p class="form-row form-group form-row-wide">
    <label for="reg_password"><?php esc_html_e( 'Mật khẩu', 'dokan-lite' ); ?> <span class="required">*</span></label>
    <input type="password" class="input-text form-control" name="password" id="reg_password"
      value="<?php echo ! empty( $data['password'] ) ? esc_attr( $data['password'] ) : ''; ?>" required="required"
      minlength="6" />
  </p>
  <?php endif; ?>
  ...
  <div style="left:-999em; position:absolute;"><label
      for="trap"><?php esc_html_e( 'Chống thư rác (Anti-spam)', 'dokan-lite' ); ?></label><input type="text"
      name="email_2" id="trap" tabindex="-1" /></div>
  ...
  <p class="form-row form-group form-row-wide">
    <label for="seller-url" class="pull-left"><?php esc_html_e( 'Đường dẫn đến cửa hàng', 'dokan-lite' ); ?> <span
        class="required">*</span></label>
  ...
  <p class="form-row">
    ...
    <input type="submit" class="dokan-btn dokan-btn-theme" name="register"
      value="<?php esc_attr_e( 'Register', 'dokan-lite' ); ?>" />
  </p>
