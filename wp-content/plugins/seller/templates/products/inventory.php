<div class="dokan-product-inventory dokan-edit-row <?php echo esc_attr( $class ); ?>">
  <div class="dokan-section-heading" data-togglehandler="dokan_product_inventory">
    <h2><i class="fas fa-cubes" aria-hidden="true"></i> <?php esc_html_e( 'Hàng tồn kho', 'dokan-lite' ); ?></h2>
    <p><?php esc_html_e( 'Quản lý hàng tồn kho cho sản phẩm này.', 'dokan-lite' ); ?></p>
  ...
  <div class="dokan-section-content">
    ...
    <div class="content-half-part dokan-form-group">
      <label for="_sku" class="form-label"><?php esc_html_e( 'Số lượng sản phẩm đang có trong kho', 'dokan-lite' ); ?>
        <span><?php esc_html_e( 'Stock Keeping Unit (SKU)', 'dokan-lite' ); ?></span></label>
    ...
    <div class="content-half-part hide_if_variable hide_if_external hide_if_stock_global">
      <label for="_stock_status"
        class="form-label"><?php esc_html_e( 'Trạng thái sản phẩm trong kho', 'dokan-lite' ); ?></label>
    ...
    <div class="show_if_stock dokan-stock-management-wrapper dokan-form-group dokan-clearfix">
      ...
      <div class="content-half-part">
        <label for="_stock" class="form-label"><?php esc_html_e( 'Số lượng hàng tồn kho', 'dokan-lite' ); ?></label>
        <input type="number" class="dokan-form-control" name="_stock"
          placeholder="<?php esc_attr__( '1', 'dokan-lite' ); ?>"
          value="<?php echo esc_attr( wc_stock_amount( $_stock ) ); ?>" min="0" step="1">
      </div>

      <?php if ( version_compare( WC_VERSION, '3.4.7', '>' ) ) : ?>
      <div class="content-half-part">
        <label for="_low_stock_amount"
          class="form-label"><?php esc_html_e( 'Ngưỡng tồn kho thấp', 'dokan-lite' ); ?></label>
        <input type="number" class="dokan-form-control" name="_low_stock_amount"
          placeholder="<?php esc_attr__( '1', 'dokan-lite' ); ?>"
          value="<?php echo esc_attr( wc_stock_amount( $_low_stock_amount ) ); ?>" min="0" step="1">
      </div>
      <?php endif; ?>

      <div class="content-half-part last-child">
        <label for="_backorders"
          class="form-label"><?php esc_html_e( 'Cho phép đặt hàng sau (Allow Backorders)', 'dokan-lite' ); ?></label>
    ...
    <div class="dokan-form-group hide_if_grouped hide_if_external">
      <label class="" for="_sold_individually">
        <input name="_sold_individually" id="_sold_individually" value="yes" type="checkbox"
          <?php checked( $_sold_individually, 'yes' ); ?>>
        <?php esc_html_e( 'Chỉ cho phép khách hàng mua một sản phẩm này trong một lần mua hàng', 'dokan-lite' ); ?>
      </label>
    </div>
