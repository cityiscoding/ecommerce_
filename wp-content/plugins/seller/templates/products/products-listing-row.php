...
<tr class="<?php echo esc_attr( $tr_class ); ?>">
  <th class="dokan-product-select check-column">
    ...
    <input class="cb-select-items dokan-checkbox" type="checkbox"
      data-product-name="<?php echo esc_attr( $product->get_title() ); ?>" name="bulk_products[]"
      value="<?php echo esc_attr( $post->ID ); ?>">
  </th>
  <td data-title="<?php esc_attr_e( 'Image', 'dokan-lite' ); ?>" class="column-thumb">
    <?php if ( current_user_can( 'dokan_edit_product' ) ) { ?>
    <a
      href="<?php echo esc_url( dokan_edit_product_url( $post->ID ) ); ?>"><?php echo wp_kses( $product->get_image( 'thumbnail' ), $img_kses ); ?></a>
    <?php } else { ?>
  ...
  <td data-title="<?php esc_attr_e( 'Name', 'dokan-lite' ); ?>" class="column-primary">
    <?php if ( current_user_can( 'dokan_edit_product' ) ) { ?>
    <strong><a
        href="<?php echo esc_url( dokan_edit_product_url( $post->ID ) ); ?>"><?php echo esc_html( $product->get_title() ); ?></a></strong>
    <?php } else { ?>
  ...
  <td class="post-status" data-title="<?php esc_attr_e( 'Status', 'dokan-lite' ); ?>">
    <label
      class="dokan-label <?php echo esc_attr( dokan_get_post_status_label_class( $post->post_status ) ); ?>"><?php echo esc_html( dokan_get_post_status( $post->post_status ) ); ?></label>
  </td>
  ...
  <td class="post-date" data-title="<?php esc_attr_e( 'Date', 'dokan-lite' ); ?>">
    <?php
        ...
        if ( 'publish' === $post->post_status ) {
            esc_html_e( 'Được phát hành', 'dokan-lite' );
        } elseif ( 'future' === $post->post_status ) {
            if ( $time_diff > 0 ) {
                echo '<strong class="attention">' . esc_html__( 'Missed schedule', 'dokan-lite' ) . '</strong>';
            } else {
                esc_html_e( 'Lên kế hoạch', 'dokan-lite' );
            }
        } else {
            esc_html_e( 'Sửa đổi lần cuối', 'dokan-lite' );
        }
        ?>
    </div>
  </td>
