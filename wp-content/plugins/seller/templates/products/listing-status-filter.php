...
<ul class="dokan-listing-filter dokan-left subsubsub">
  <li<?php echo $status_class === 'all' ? ' class="active"' : ''; ?>>
    ...
    <li<?php echo $status_class === $status ? ' class="active"' : ''; ?>>
      <a
        href="<?php echo esc_url( add_query_arg( [ 'post_status' => $status, '_product_listing_filter_nonce' => $nonce ], $permalink ) ); ?>"><?php echo esc_html( $status_label ) . ' (' . esc_html( $post_counts->{$status} ) . ')'; ?></a>
      <?php // phpcs:ignore ?>
      </li>
      <?php endforeach ?>
      ...
      <li<?php echo $status_class === 'instock' ? ' class="active"' : ''; ?>>
        <a
          href="<?php echo esc_url( add_query_arg( [ 'post_status' => 'instock', '_product_listing_filter_nonce' => $nonce ], $permalink ) ); ?>"><?php echo esc_html_e( 'Còn hàng', 'dokan-lite' ) . ' (' . esc_html( $instock_counts ) . ')'; ?></a>
        <?php // phpcs:ignore ?>
        </li>
        <?php endif; ?>
        ...
        <li<?php echo $status_class === 'outofstock' ? ' class="active"' : ''; ?>>
          <a
            href="<?php echo esc_url( add_query_arg( [ 'post_status' => 'outofstock', '_product_listing_filter_nonce' => $nonce ], $permalink ) ); ?>"><?php echo esc_html_e( 'Hết hàng', 'dokan-lite' ) . ' (' . esc_html( $outofstock_counts ) . ')'; ?></a>
          <?php // phpcs:ignore ?>
          </li>
          <?php endif; ?>
