  ...
  <div class="dokan-dashboard-content dokan-product-edit">
      ...
      <h1 class="entry-title">
        <?php
                    if ( $new_product || 'auto-draft' === $post->post_status ) {
                        esc_html_e( 'Thêm sản phẩm mới', 'dokan-lite' );
                    } else {
                        esc_html_e( 'Chỉnh sửa sản phẩm', 'dokan-lite' );
                    }
                    ?>
        <span
          class="dokan-label <?php echo esc_attr( dokan_get_post_status_label_class( $post->post_status ) ); ?> dokan-product-status-label">
          <?php echo esc_html( dokan_get_post_status( $post->post_status ) ); ?>
        </span>

        <?php if ( $post->post_status === 'publish' ) : ?>
        <span class="dokan-right">
          <a class="dokan-btn dokan-btn-theme dokan-btn-sm" href="<?php echo esc_url( get_permalink( $post->ID ) ); ?>"
            target="_blank"><?php esc_html_e( 'Xem sản phẩm', 'dokan-lite' ); ?></a>
        </span>
        <?php endif; ?>

        <?php if ( $_visibility === 'hidden' ) : ?>
        <span class="dokan-right dokan-label dokan-label-default dokan-product-hidden-label"><i
            class="far fa-eye-slash"></i> <?php esc_html_e( 'Ẩn', 'dokan-lite' ); ?></span>
        <?php endif; ?>
      </h1>
    </header><!-- .entry-header -->
    ...
    <div class="product-edit-new-container product-edit-container">
      ...
      <div class="dokan-alert dokan-alert-danger">
        ...
        <strong><?php esc_html_e( 'Lỗi!', 'dokan-lite' ); ?></strong> <?php echo esc_html( $error ); ?>.<br>
        <?php endforeach; ?>
      </div>
      <?php endif; ?>
      ...
      <div class="dokan-message">
        ...
        <strong><?php esc_html_e( 'Thành công!', 'dokan-lite' ); ?></strong>
        <?php esc_html_e( 'Sản phẩm đã được lưu thành công.', 'dokan-lite' ); ?>

        <?php if ( $post->post_status === 'publish' ) : ?>
        <a href="<?php echo esc_url( get_permalink( $post_id ) ); ?>"
          target="_blank"><?php esc_html_e( 'Xem sản phẩm &rarr;', 'dokan-lite' ); ?></a>
        <?php endif; ?>
      </div>
      <?php endif; ?>
      ...
      <form class="dokan-product-edit-form" role="form" method="post" id="post">
        ...
        <div class="dokan-form-top-area">
          ...
          <div class="content-half-part dokan-product-meta">
            ...
            <div id="dokan-product-title-area" class="dokan-form-group">
              ...
              <label for="post_title" class="form-label"><?php esc_html_e( 'Tiêu đề', 'dokan-lite' ); ?></label>
              <?php
              ...
              <div class="dokan-product-title-alert dokan-hide">
                <?php esc_html_e( 'Vui lòng nhập tiêu đề sản phẩm!', 'dokan-lite' ); ?>
              </div>
              ...
              <input type="hidden" value="<?php echo esc_attr( $post->post_name ); ?>" name="edited-post-name"
                class="dokan-hide" id="edited-post-name-dokan">
            </div>
            ...
            <div class="dokan-form-group">
              <label for="product_type" class="form-label"><?php esc_html_e( 'Loại sản phẩm', 'dokan-lite' ); ?> <i
                  class="fas fa-question-circle tips" aria-hidden="true"
                  data-title="<?php esc_html_e( 'Chọn nếu sản phẩm của bạn có nhiều thuộc tính - như kích thước, màu sắc, chất lượng, v.v.', 'dokan-lite' ); ?>"></i></label>
            ...
            <div class="show_if_simple dokan-clearfix show_if_external">
              ...
              <div class="dokan-form-group dokan-clearfix dokan-price-container">
                ...
                <div class="content-half-part regular-price">
                  <label for="_regular_price" class="form-label"><?php esc_html_e( 'Giá', 'dokan-lite' ); ?></label>
                  <div class="dokan-input-group">
                    <span
                      class="dokan-input-group-addon"><?php echo esc_html( get_woocommerce_currency_symbol() ); ?></span>
                    <?php
                                                    dokan_post_input_box(
                                                        $post_id,
                                                        '_regular_price',
                ...
                <div class="content-half-part sale-price">
                  <label for="_sale_price" class="form-label">
                    <?php esc_html_e( 'Giá ưu đãi', 'dokan-lite' ); ?>
                    <a href="#"
                      class="sale_schedule <?php echo $show_schedule ? 'dokan-hide' : ''; ?>"><?php esc_html_e( 'Lịch trình giảm giá', 'dokan-lite' ); ?></a>
                    <a href="#"
                      class="cancel_sale_schedule <?php echo ( ! $show_schedule ) ? 'dokan-hide' : ''; ?>"><?php esc_html_e( 'Hủy bỏ', 'dokan-lite' ); ?></a>
                  </label>
              ...
              <div class="dokan-form-group dokan-clearfix dokan-price-container">
                <div class="dokan-product-less-price-alert dokan-hide">
                  <?php esc_html_e( 'Giá sản phẩm không thể thấp hơn giảm giá!', 'dokan-lite' ); ?>
                </div>
              </div>

              <div
                class="sale_price_dates_fields dokan-clearfix dokan-form-group <?php echo ( ! $show_schedule ) ? 'dokan-hide' : ''; ?>">
                <div class="content-half-part from">
                  <div class="dokan-input-group">
                    <span class="dokan-input-group-addon"><?php esc_html_e( 'Từ', 'dokan-lite' ); ?></span>
                    <input type="text" name="_sale_price_dates_from" class="dokan-form-control dokan-start-date"
                      value="<?php echo esc_attr( $_sale_price_dates_from ); ?>" maxlength="10"
                      pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])"
                      placeholder="<?php esc_html_e( 'Năm-Tháng-Ngày', 'dokan-lite' ); ?>">
                  </div>
                </div>

                <div class="content-half-part to">
                  <div class="dokan-input-group">
                    <span class="dokan-input-group-addon"><?php esc_html_e( 'Đến', 'dokan-lite' ); ?></span>
                    <input type="text" name="_sale_price_dates_to" class="dokan-form-control dokan-end-date"
                      value="<?php echo esc_attr( $_sale_price_dates_to ); ?>" maxlength="10"
                      pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])"
                      placeholder="<?php esc_html_e( 'Năm-Tháng-Ngày', 'dokan-lite' ); ?>">
                  </div>
                </div>
            ...
            <div class="dokan-form-group">
              <label for="product_tag_edit" class="form-label"><?php esc_html_e( 'Thẻ', 'dokan-lite' ); ?></label>
              <?php
                                        ...
                                        ?>
              <select multiple="multiple" id="product_tag_edit" name="product_tag[]"
                class="product_tag_search dokan-form-control"
                data-placeholder="<?php echo esc_attr( $tags_placeholder ); ?>">
                <?php if ( ! empty( $terms ) ) : ?>
            ...
            <div class="dokan-feat-image-upload dokan-new-product-featured-img">
              ...
              <div class="instruction-inside<?php echo esc_attr( $instruction_class ); ?>">
                <input type="hidden" name="feat_image_id" class="dokan-feat-image-id"
                  value="<?php echo esc_attr( $feat_image_id ); ?>">

                <i class="fas fa-cloud-upload-alt"></i>
                <a href="#"
                  class="dokan-feat-image-btn btn btn-sm"><?php esc_html_e( 'Tải ảnh sản phẩm', 'dokan-lite' ); ?></a>
              </div>
                  ...
                  <ul class="product_images dokan-clearfix">
                    ...
                    <li class="image" data-attachment_id="<?php echo esc_attr( $image_id ); ?>">
                      ...
                      <a href="#" class="action-delete"
                        title="<?php esc_attr_e( 'Delete image', 'dokan-lite' ); ?>">&times;</a>
                    </li>
                    <?php
                                                            endforeach;
                                                        endif;
                    ...
                    <li class="add-image add-product-images tips"
                      data-title="<?php esc_html_e( 'Thêm hình ảnh', 'dokan-lite' ); ?>">
        ...
        <div class="dokan-product-short-description">
          <label for="post_excerpt" class="form-label"><?php esc_html_e( 'Mô tả ngắn', 'dokan-lite' ); ?></label>
          <?php
                                wp_editor(
                                    $post_excerpt,
                                    'post_excerpt',
        ...
        <div class="dokan-product-description">
          <label for="post_content" class="form-label"><?php esc_html_e( 'Miêu tả chi tiết', 'dokan-lite' ); ?></label>
          <?php
                                wp_editor(
                                    $post_content,
                                    'post_content',
