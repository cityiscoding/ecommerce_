...
<div class="dokan-edit-row dokan-catalog-mode dokan-clearfix">
  <div class="dokan-section-heading">
    <h2>
      ...
      <?php esc_html_e( 'Chỉ xem (Catalog Mode)', 'dokan-lite' ); ?>
    </h2>
    <p><?php esc_html_e( 'Kích hoạt/Ngừng kích hoạt chế độ danh mục cho sản phẩm này', 'dokan-lite' ); ?></p>
  ...
  <div class="dokan-section-content">
    ...
    <label for="catalog_mode_hide_add_to_cart_button" style="display: block;">
      <input type="checkbox" id="catalog_mode_hide_add_to_cart_button" value="on"
        name="catalog_mode[hide_add_to_cart_button]"
        <?php checked( $saved_data['hide_add_to_cart_button'], 'on' ); ?> />
      <span>
        <?php esc_html_e( 'Ấn để xóa tùy chọn "Thêm sp vào giỏ hàng" khỏi sản phẩm của bạn.', 'dokan-lite' ); ?></span>
    </label>
    <?php endif; ?>
    <?php if ( Helper::hide_product_price_option_is_enabled_by_admin() ) : ?>
    <label for="catalog_mode_hide_product_price"
      style="display: <?php echo 'on' === $saved_data['hide_add_to_cart_button'] ? 'none' : 'block'; ?>;">
      ...
      <span> <?php esc_html_e( 'Ấn để ẩn giá sản phẩm khỏi sản phẩm của bạn.', 'dokan-lite' ); ?></span>
    </label>
    <?php endif; ?>
    <div class="dokan-clearfix"></div>
  </div>
