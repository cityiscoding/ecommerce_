...
<body class="wc-setup wp-core-ui dokan-vendor-setup-wizard">
  ...
  <a class="wc-return-to-dashboard"
    href="<?php echo esc_url( site_url() ); ?>"><?php esc_attr_e( 'Return to the Marketplace', 'dokan-lite' ); ?></a>
  <?php endif; ?>
</body>
...
<?php endif; ?>
<p>
  <?php esc_attr_e( 'No time right now? If you don’t want to go through the wizard, you can skip and return to the Store!', 'dokan-lite' ); ?>
</p>
<p class="wc-setup-actions step">
  <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
    class="button-primary button button-large button-next lets-go-btn dokan-btn-theme"><?php esc_attr_e( 'Let\'s Go!', 'dokan-lite' ); ?></a>
  <a href="<?php echo esc_url( $dashboard_url ); ?>"
    class="button button-large not-right-now-btn dokan-btn-theme"><?php esc_attr_e( 'Not right now', 'dokan-lite' ); ?></a>
</p>
<?php
    ...
    public function dokan_setup_store() {
        ...
        ?>
<h1><?php esc_attr_e( 'Thiết lập cơ bản cho cửa hàng của bạn', 'dokan-lite' ); ?></h1>
<form method="post" class="dokan-seller-setup-form">
  <table class="form-table">
    <tr>
      <th scope="row">
        <label for="address[street_1]">
          <?php esc_html_e( 'Địa chỉ cụ thể', 'dokan-lite' ); ?>
      ...
      <td>
        <input type="text" id="address[street_1]" name="address[street_1]"
          value="<?php echo esc_attr( $address_street1 ); ?>" />
        <span class="error-container">
          <?php
                            if ( ! empty( $_POST['error_address[street_1]'] ) ) {
                                echo '<span class="required">' . __( 'Bắt buộc', 'dokan-lite' ) . '</span>';
                            }
                            ?>
        </span>
      </td>
    ...
    <tr>
      <th scope="row">
        <label for="address[street_2]">
          <?php esc_html_e( 'Địa chỉ 2 (Không bắt buộc)', 'dokan-lite' ); ?>
        </label>
      </th>
      <td>
        <input type="text" id="address[street_2]" name="address[street_2]"
          value="<?php echo esc_attr( $address_street2 ); ?>" />
        <span class="error-container">
          <?php
                            if ( ! empty( $_POST['error_address[street_2]'] ) ) {
                                echo '<span class="required">' . __( 'Bắc buộc', 'dokan-lite' ) . '</span>';
                            }
                            ?>
        </span>
      </td>
    ...
    <tr>
      <th scope="row">
        <label for="calc_shipping_state">
          <?php esc_html_e( 'Tiểu bang (bỏ qua nếu ở Việt Nam)', 'dokan-lite' ); ?>
          <span class='required'>*</span></label>
      </th>
      </label>
      </th>
      <td>
        <input type="text" id="calc_shipping_state" name="address[state]"
          value="<?php echo esc_attr( $address_state ); ?>" /
          placeholder="<?php esc_attr_e( 'Hoa kỳ, Mexico, Úc, Brazil, Venezuela,Ấn Độ, Malaysia', 'dokan-lite' ); ?>">
        <span class="error-container">
          <?php
    ...
    <tr>
      <th scope="row"><label for="show_email"><?php esc_html_e( 'Tùy chọn hiển thị Email', 'dokan-lite' ); ?></label>
      </th>
      <td class="checkbox">
        <input type="checkbox" name="show_email" id="show_email" class="switch-input" value="1"
          <?php echo ( $show_email === 'yes' ) ? 'checked="true"' : ''; ?>>
  ...
  <p class="wc-setup-actions step">
    <input type="button" class="button-primary button button-large button-next store-step-continue dokan-btn-theme"
    ...
    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
      class="button button-large button-next store-step-skip-btn dokan-btn-theme"><?php esc_html_e( 'Skip this step', 'dokan-lite' ); ?></a>
  ...
  $('body').on('change', 'select.country_to_state, input.country_to_state', function() {
    ...
    if (states[country]) {
      if ($.isEmptyObject(states[country])) {
        ...
        $statebox.replaceWith('<input type="hidden" class="hidden" name="' + input_name + '" id="' + input_id +
        ...
        if ($statebox.is('input')) {
          // Change for select
          $statebox.replaceWith('<select name="' + input_name + '" id="' + input_id +
            '" class="wc-enhanced-select state_select" data-placeholder="' + placeholder + '"></select>');
          $statebox = $wrapper.find('#calc_shipping_state');
        }
    ...
    requiredFields.map(item => {
      if (!$(`*[name='${item}']`).val()) {
        $(`*[name='${item}']`).closest('td').children(`span.error-container`).html(
          `<span class="required">${requiredMsg}</span>`);
      } else {
        $(`*[name='${item}']`).closest('td').children(`span.error-container`).html('');
      }
    });

    if (('object' === typeof states[data['address[country]']] && Object.keys(states[data['address[country]']])
        .length && !data['address[state]']) || ('undefined' === typeof states[data['address[country]']] && !data[
        'address[state]'])) {
...
<form method="post" id='dokan-seller-payment-setup-form' novalidate>
  ...
  <p class="wc-setup-actions step">
    <input type="submit" class="button-primary button button-large button-next payment-continue-btn dokan-btn-theme"
      value="<?php esc_attr_e( 'Continue', 'dokan-lite' ); ?>" name="save_step" />

    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
      class="button button-large button-next payment-step-skip-btn dokan-btn-theme"><?php esc_html_e( 'Skip this step', 'dokan-lite' ); ?></a>
    <?php wp_nonce_field( 'dokan-seller-setup' ); ?>
  </p>
</form>
<?php
...
<div class="dokan-setup-done-content">
  <p class="wc-setup-actions step">
    <a class="button button-primary dokan-btn-theme"
      href="<?php echo esc_url( $dashboard_url ); ?>"><?php esc_html_e( 'Go to your Store Dashboard!', 'dokan-lite' ); ?></a>
  </p>
</div>
<?php
    }
