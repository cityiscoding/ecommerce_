...
class UserProfile {
    ...
    public function enqueue_scripts( $page ) {
        if ( in_array( $page, array( 'profile.php', 'user-edit.php' ), true ) ) {
            ...
            $admin_admin_script = array(
                ...
                'seller'      => array(
                    'available'    => __( 'Hợp lệ', 'dokan-lite' ),
                    'notAvailable' => __( 'Không hợp lệ', 'dokan-lite' ),
                ),
            );
    ...
    public function add_meta_fields( $user ) {
        ...
        $country_state = array(
            ...
            'state' => array(
                'label'       => __( 'Tỉnh', 'dokan-lite' ),
                'description' => __( 'Tỉnh', 'dokan-lite' ),
...
<table class="form-table">
  <tbody>
      ...
      <td>
        <div class="dokan-banner">
          <div class="image-wrap<?php echo esc_attr( $banner ) ? '' : ' dokan-hide'; ?>">
            ...
            <input type="hidden" class="dokan-file-field" value="<?php echo esc_attr( $banner ); ?>"
              name="dokan_banner">
          ...
          <div class="button-area<?php echo esc_attr( $banner ) ? ' dokan-hide' : ''; ?>">
            <a href="#"
              class="dokan-banner-drag button button-primary"><?php esc_html_e( 'Tải lên ảnh bìa người bán', 'dokan-lite' ); ?></a>
            <p class="description">
              <?php
                                    echo sprintf(
                                        /* translators: %1$s: banner width, %2$s: banner height in integers */
                                        esc_attr__( 'Tải ảnh bìa của người bán. Kích thước tối ưu là(%1$sx%2$s) pixels.', 'dokan-lite' ),
    ...
    <tr>
      <th><?php esc_html_e( 'Tên cửa hàng', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" name="dokan_store_name" class="regular-text"
          value="<?php echo esc_attr( $store_settings['store_name'] ); ?>">
      </td>
    </tr>

    <tr>
      <th><?php esc_html_e( 'Đường dẫn đến cửa hàng (URL Shop)', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" name="dokan_store_url" data-vendor="<?php echo esc_attr( $user->ID ); ?>"
          class="regular-text" id="seller-url" value="<?php echo esc_attr( $user->data->user_nicename ); ?>"><strong
          id="url-alart-mgs"></strong>
        <p><small><?php echo esc_url( home_url() . '/' . dokan_get_option( 'custom_store_url', 'dokan_general', 'store' ) ); ?>/<strong
              id="url-alart"><?php echo esc_attr( $user->data->user_nicename ); ?></strong></small></p>
      </td>
    </tr>

    <tr>
      <th><?php esc_html_e( 'Địa chỉ 1 (địa chỉ cụ thể)', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" name="dokan_store_address[street_1]" class="regular-text"
          value="<?php echo esc_attr( $address_street1 ); ?>">
      </td>
    </tr>

    <tr>
      <th><?php esc_html_e( 'Địa chỉ 2 (Không bắt buộc', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" name="dokan_store_address[street_2]" class="regular-text"
          value="<?php echo esc_attr( $address_street2 ); ?>">
      </td>
    </tr>

    <tr>
      <th><?php esc_html_e( 'Thành phố trực thuộc / Thị trấn', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" name="dokan_store_address[city]" class="regular-text"
          value="<?php echo esc_attr( $address_city ); ?>">
      </td>
    </tr>

    <tr>
      <th><?php esc_html_e( 'Mã bưu điện', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" name="dokan_store_address[zip]" class="regular-text"
    ...
    <tr>
      ...
      <td>
        ...
        <select name="dokan_store_address[<?php echo esc_attr( $key ); ?>]" id="<?php echo esc_attr( $key ); ?>"
          class="<?php echo ( ! empty( $field['class'] ) ? esc_attr( $field['class'] ) : '' ); ?>" style="width: 25em;">
          <?php
        ...
        <?php
                                ...
                                ?>
        <input type="text" name="dokan_store_address[<?php echo esc_attr( $key ); ?>]"
    ...
    <tr>
      <th><?php esc_html_e( 'Số điện thoại', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" name="dokan_store_phone" class="regular-text"
          value="<?php echo esc_attr( $store_settings['phone'] ); ?>">
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <input type="text" name="dokan_social[<?php echo esc_attr( $key ); ?>]" class="regular-text"
    ...
    <tr>
      <th><?php esc_html_e( 'Tùy chỉnh thông tin thanh toán : ', 'dokan-lite' ); ?></th>
    </tr>
    ...
    <tr>
      ...
      <td>
        <input type="text" disabled class="regular-text"
          value="<?php echo esc_attr( $store_settings['payment']['paypal']['email'] ); ?>">
      </td>
    </tr>
    <?php } ?>
    ...
    <tr>
      ...
      <td>
        <input type="text" disabled class="regular-text"
          value="<?php echo esc_attr( $store_settings['payment']['skrill']['email'] ); ?>">
      </td>
    </tr>
    <?php } ?>
    ...
    <tr>
      <th><?php esc_html_e( 'Tên ngân hàng ', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" disabled class="regular-text"
          value="<?php echo isset( $store_settings['payment']['bank']['bank_name'] ) ? esc_attr( $store_settings['payment']['bank']['bank_name'] ) : ''; ?>">
      </td>
    </tr>
    <tr>
      <th><?php esc_html_e( 'Tên tài khoản', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" disabled class="regular-text"
          value="<?php echo isset( $store_settings['payment']['bank']['ac_name'] ) ? esc_attr( $store_settings['payment']['bank']['ac_name'] ) : ''; ?>">
      </td>
    </tr>
    <tr>
      <th><?php esc_html_e( 'Số tài khoản ', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" disabled class="regular-text"
          value="<?php echo isset( $store_settings['payment']['bank']['ac_number'] ) ? esc_attr( $store_settings['payment']['bank']['ac_number'] ) : ''; ?>">
      </td>
    </tr>
    <tr>
      <th><?php esc_html_e( 'Địa chỉ ngân hàng gần nhất', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" disabled class="regular-text"
          value="<?php echo isset( $store_settings['payment']['bank']['bank_addr'] ) ? esc_attr( $store_settings['payment']['bank']['bank_addr'] ) : ''; ?>">
      </td>
    </tr>
    <tr>
      <th><?php esc_html_e( 'Số định tuyến (Không bắt buộc)', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" disabled class="regular-text"
          value="<?php echo isset( $store_settings['payment']['bank']['routing_number'] ) ? esc_attr( $store_settings['payment']['bank']['routing_number'] ) : ''; ?>">
      </td>
    </tr>
    <tr>
      <th><?php esc_html_e( 'Mã IBAN ngân hàng (Không bắt buộc) ', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" disabled class="regular-text"
          value="<?php echo isset( $store_settings['payment']['bank']['iban'] ) ? esc_attr( $store_settings['payment']['bank']['iban'] ) : ''; ?>">
      </td>
    </tr>
    <tr>
      <th><?php esc_html_e( 'Mã Swift Code ngân hàng ', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" disabled class="regular-text"
          value="<?php echo isset( $store_settings['payment']['bank']['swift'] ) ? esc_attr( $store_settings['payment']['bank']['swift'] ) : ''; ?>">
      </td>
    </tr>
    <?php } ?>
    <tr>
      ...
      <td>
        <label for="dokan_enable_selling">
          ...
          <input name="dokan_enable_selling" type="checkbox" id="dokan_enable_selling" value="yes"
            <?php checked( $selling, 'yes' ); ?> />
          <?php esc_html_e( 'Bật thêm sản phẩm', 'dokan-lite' ); ?>
        </label>

        <p class="description"><?php esc_html_e( 'Bật hoặc tắt khả năng thêm sản phẩm', 'dokan-lite' ); ?></p>
      </td>
    </tr>

    <tr>
      <th><?php esc_html_e( 'Xuất bản', 'dokan-lite' ); ?></th>
      <td>
        <label for="dokan_publish">
          <input type="hidden" name="dokan_publish" value="no">
          <input name="dokan_publish" type="checkbox" id="dokan_publish" value="yes"
            <?php checked( $publishing, 'yes' ); ?> />
          <?php esc_html_e( 'Xuất bản sản phẩm trực tiếp', 'dokan-lite' ); ?>
        </label>

        <p class="description">
          <?php esc_html_e( 'Bỏ qua đang chờ xử lý, xuất bản sản phẩm trực tiếp', 'dokan-lite' ); ?></p>
      </td>
    </tr>

    <tr>
      <th><?php esc_html_e( 'Loại hoa hồng quản trị ', 'dokan-lite' ); ?></th>
      <td>
        <select id="dokan_admin_percentage_type" name="dokan_admin_percentage_type">
          <?php foreach ( dokan_commission_types() as $key => $value ) : ?>
          <option value="<?php echo esc_attr( $key ); ?>" <?php selected( $admin_commission_type, $key ); ?>>
            <?php echo esc_attr( $value ); ?></option>
          <?php endforeach; ?>
        </select>
        <p class="description">
          <?php esc_html_e( 'Đặt loại hoa hồng mà quản trị viên nhận được từ người bán này', 'dokan-lite' ); ?></p>
      </td>
    </tr>
    <tr>
      <th><?php esc_html_e( 'Hoa hồng cho quản trị viên ', 'dokan-lite' ); ?></th>
      <td>
        <input type="text" class="wc_input_price small-text" id="admin-commission" name="dokan_admin_percentage"
          value="<?php echo esc_attr( $admin_commission ); ?>">
        <?php do_action( 'dokan_seller_meta_fields_after_admin_commission', $user ); ?>
        <p class="combine-commission-description">
          <?php esc_html_e( 'Nó sẽ ghi đè hoa hồng mặc định mà quản trị viên nhận được từ mỗi lần bán hàng', 'dokan-lite' ); ?>
        </p>
      </td>
    </tr>

    <tr>
      <th><?php esc_html_e( 'Nhà cung cấp nổi bật', 'dokan-lite' ); ?></th>
      <td>
        <label for="dokan_feature">
          <input type="hidden" name="dokan_feature" value="no">
          <input name="dokan_feature" type="checkbox" id="dokan_feature" value="yes"
            <?php checked( $feature_seller, 'yes' ); ?> />
          <?php esc_html_e( 'làm cho nhà cung cấp nổi bật', 'dokan-lite' ); ?>
        </label>

        <p class="description">
          <?php esc_html_e( 'Nhà cung cấp này sẽ được đánh dấu là nhà cung cấp nổi bật.', 'dokan-lite' ); ?>
        </p>
      </td>
    </tr>
...
jQuery(function($) {
  ...
  $('#seller-url').on('keydown', function(e) {
    ...
    if ((e.shiftKey || (e.keyCode < 65 || e.keyCode > 90) && (e.keyCode < 48 || e.keyCode > 57)) && (e.keyCode <
        96 || e.keyCode > 105)) {
      e.preventDefault();
    }
  });
  ...
  $('#seller-url').on('focusout', function() {
    ...
    row.block({
      message: null,
      overlayCSS: {
        background: '#f1f1f1 url(' + dokan_user_profile.ajax_loader + ') no-repeat center',
        opacity: 0.3
      }
    });
    ...
    $.post(dokan_user_profile.ajaxurl, data, function(resp) {
      ...
      if (resp.success === true) {
        ...
        $('#url-alart-mgs').removeClass('text-danger').addClass('text-success').text(dokan_user_profile
          .seller.available);
      } else {
        $('#url-alart').removeClass('text-success').addClass('text-danger');
        $('#url-alart-mgs').removeClass('text-success').addClass('text-danger').text(dokan_user_profile
          .seller.notAvailable);
      }
