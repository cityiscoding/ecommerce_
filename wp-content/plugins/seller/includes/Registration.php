...
class Registration {
    ...
    public function validate_registration( $error ) {
        ...
        $required_fields = apply_filters(
            'dokan_seller_registration_required_fields', [
                'fname'    => __( 'Vui lòng nhập tên của bạn.', 'dokan-lite' ),
                'lname'    => __( 'Vui lòng nhập họ của bạn.', 'dokan-lite' ),
                'phone'    => __( 'Xin vui lòng điền số điện thoại của bạn.', 'dokan-lite' ),
                'shopname' => __( 'Vui lòng cung cấp một tên cửa hàng.', 'dokan-lite' ),
                'shopurl'  => __( 'Vui lòng cung cấp một url cửa hàng.', 'dokan-lite' ),
            ]
        );
        ...
        if ( $role === 'seller' ) {
            ...
            if ( ! empty( get_user_by( 'slug', $shop_url ) ) ) {
                return new WP_Error( 'shop-url-error', __( 'URL cửa hàng không hợp lệ hoặc đã tồn tại', 'dokan-lite' ) );
            }
        }
    ...
    $data['first_name']    = isset( $_POST['fullname'] ) ? sanitize_text_field( wp_unslash( $_POST['fullname'] ) ) : '';
    $data['last_name']     = ''; // No need to set last name if using full name
