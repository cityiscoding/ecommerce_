...
function create_ship($order_id, bool $from_checkout)
{
    ...
    if ($rs->Code > 0) {
    $tracking_number = $rs->Data->TrackingNumber;
    
    // Bắt đầu biến $order_note với dòng mới trước khi đến button
    $order_note = __('Mã vận đơn', 'ship-depot-translate') . ' ' . $tracking_number . '<br>';

    if ($from_checkout) {
        // Thêm button vào ghi chú đơn hàng
        $order_note .= '<button style="text-transform: uppercase;"><a href="https://donhang.ghn.vn/?order_code=' . $tracking_number . '" target="_blank">' . __('Xem thông tin vận đơn', 'ship-depot-translate') . '</a></button>';
    }

    // Thêm ghi chú vào đơn hàng
    $order->add_order_note($order_note);
    // Cập nhật thông tin vận đơn vào metadata của đơn hàng
    Ship_Depot_Helper::UpdateOrderMetadata($order_id, 'sd_ship_info', json_encode($rs->Data, JSON_UNESCAPED_UNICODE));
    Ship_Depot_Helper::UpdateOrderMetadata($order_id, 'sd_from_fe', json_encode(false));
