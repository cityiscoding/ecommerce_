...
class ReverseWithdrawal {
    ...
    public function send_balance_threshold_exceeded_announcement( $vendor_id, $due_status ) {
        ...
        $message = sprintf(
            ...
            __( 'Bạn có số dư rút ngược là %1$s cần được thanh toán. Vui lòng <a href="%2$s">thanh toán</a> trước %3$s.', 'dokan' ),
        ...
        if ( is_wp_error( $notice ) ) {
            dokan_log( sprintf( 'Rút tiền ngược: Thông báo tạo lỗi cho người bán %1$s, Thông báo lỗi: %2$s', $vendor_id, $notice->get_error_message() ) );
        }
    }
