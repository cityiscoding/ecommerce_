...
<span class="additional-fee" style="display: none;">
  <?php echo esc_html( '% &nbsp;&nbsp; +' ); ?>
  <input type="text" class="wc_input_price small-text" name="dokan_admin_additional_fee"
    value="<?php echo esc_attr( wc_format_localized_price( $additional_fee ) ); ?>">
</span>
...
(function($) {
  $('select[name=commission_type]').on('change', function() {
    if ('combine' === $(this).val()) {
      ...
      $('.combine-commission-description').text(
        '<?php echo esc_html__( 'Amount you will get from sales in both percentage and fixed fee', 'dokan' ); ?>'
      );
      $('input[name=admin_percentage]').css({
        'width': '100px',
        'display': 'inline'
      });
      ...
      $('.combine-commission-description').text(
        '<?php echo esc_html__( 'How much amount (%) you will get from each order', 'dokan' ); ?>');
      $('input[name=admin_percentage]').css({
        'width': '100%',
        'display': 'block'
      });
...
<?php
    ...
    public function create_default_pages() {
        ...
        $pages = array(
            array(
                'post_title' => __( 'Bảng điều khiển', 'dokan' ),
                'slug'       => 'dashboard',
                'page_id'    => 'dashboard',
    ...
    public function add_combine_commission_type( $types ) {
        $types['combine'] = __( 'Kết hợp', 'dokan' );
...
<span class="additional-fee dokan-hide">
  <?php echo esc_html( '% &nbsp;&nbsp; +' ); ?>
  <input type="text" class="wc_input_price small-text" name="dokan_admin_additional_fee"
    value="<?php echo esc_attr( wc_format_localized_price( $admin_additional_fee ) ); ?>">
</span>
...
<tr>
  ...
  <td>
    <label for="withdraw_date_limit">
      <input type="number" name="withdraw_date_limit" min="0" id="withdraw_date_limit"
        value="<?php echo esc_attr( $withdraw_date_limit ); ?>" />
    </label>

    <p class="description">
      <?php esc_html_e( 'If set, it will override global withdraw threshold days for this vendor', 'dokan' ); ?></p>
  </td>
</tr>
<?php
    }
