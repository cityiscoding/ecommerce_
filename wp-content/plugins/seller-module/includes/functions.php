...
function dokan_progressbar_translated_string( $string = '', $value = 15, $progress = 0 ) {
    ...
    if ( 100 === absint( $progress ) ) :
        $translated_string = __( 'Xin chúc mừng, hồ sơ của bạn đã hoàn thành đầy đủ', 'dokan' );
    else :
        switch ( $string ) {
            case 'profile_picture_val':
                // translators: %s%% is the progressbar progress value
                $translated_string = sprintf( __( 'Thêm hình ảnh hồ sơ để đạt được %s%% tiến trình', 'dokan' ), number_format_i18n( $value ) );
                break;

            case 'phone_val':
                // translators: %s%% is the progressbar progress value
                $translated_string = sprintf( __( 'Thêm số điện thoại để đạt được %s%% tiến trình', 'dokan' ), number_format_i18n( $value ) );
                break;

            case 'banner_val':
                // translators: %s%% is the progressbar progress value
                $translated_string = sprintf( __( 'Thêm ảnh bìa để đạt được %s%% tiến trình', 'dokan' ), number_format_i18n( $value ) );
                break;

            case 'store_name_val':
                // translators: %s%% is the progressbar progress value
                $translated_string = sprintf( __( 'Thêm tên cửa hàng để đạt được %s%% tiến trình', 'dokan' ), number_format_i18n( $value ) );
                break;

            case 'address_val':
                // translators: %s%% is the progressbar progress value
                $translated_string = sprintf( __( 'Thêm địa chỉ để đạt được %s%% tiến trình', 'dokan' ), number_format_i18n( $value ) );
                break;

            case 'payment_method_val':
                // translators: %s%% is the progressbar progress value
                $translated_string = sprintf( __( 'Thêm một phương thức thanh toán để đạt được %s%% tiến trình', 'dokan' ), number_format_i18n( $value ) );
                break;

            case 'map_val':
                // translators: %s%% is the progressbar progress value
                $translated_string = sprintf( __( 'Thêm vị trí bản đồ để đạt được %s%% tiến trình', 'dokan' ), number_format_i18n( $value ) );
                break;

            case 'fb':
                // translators: %s%% is the progressbar progress value
                $translated_string = sprintf( __( 'Thêm Facebook để đạt được %s%% tiến trình', 'dokan' ), number_format_i18n( $value ) );
                break;

            case 'twitter':
                // translators: %s%% is the progressbar progress value
                $translated_string = sprintf( __( 'Thêm Twitter để đạt được %s%% tiến trình', 'dokan' ), number_format_i18n( $value ) );
                break;

            case 'youtube':
                // translators: %s%% is the progressbar progress value
                $translated_string = sprintf( __( 'Thêm YouTube để đạt được %s%% tiến trình', 'dokan' ), number_format_i18n( $value ) );
                break;

            case 'linkedin':
                // translators: %s%% is the progressbar progress value
                $translated_string = sprintf( __( 'Thêm LinkedIn để đạt được %s%% tiến trình', 'dokan' ), number_format_i18n( $value ) );
                break;

            default:
                $translated_string = __( 'Bắt đầu với việc thêm một ảnh bìa để đạt được tiến độ hồ sơ', 'dokan' );
                break;
        }
    endif;
...
function dokan_shipping_status_tracking_providers_list() {
    $providers = [
        'sp-australia-post'            => [
            'label' => __( 'Giao hàng nhanh', 'dokan' ),
            'url'   => 'https://donhang.ghn.vn/?order_code={tracking_number}',
        ],
        'sp-australia-post1'            => [
            'label' => __( 'Giao hàng tiết kiệm', 'dokan' ),
            'url'   => 'https://i.ghtk.vn/?tracking_code={tracking_number}',
        ],
    ];
...
function dokan_get_order_shipment_status_html( $get_status ) {
    if ( 'shipped' === $get_status ) {
        return sprintf( '<span class="dokan-label dokan-label-success">%s</span>', apply_filters( 'dokan_shipment_status_label_shipped', __( 'Đã giao hàng', 'dokan' ) ) );
    } elseif ( 'partially' === $get_status ) {
        return sprintf( '<span class="dokan-label dokan-label-info">%s</span>', apply_filters( 'dokan_shipment_status_label_partially_shipped', __( 'Cung cấp một phần', 'dokan' ) ) );
    } elseif ( 'not_shipped' === $get_status ) {
        return sprintf( '<span class="dokan-label dokan-label-default">%s</span>', apply_filters( 'dokan_shipment_status_label_not_shipped', __( 'Chờ lấy hàng', 'dokan' ) ) );
    } else {
...
//hide edit shiping:
add_action('wp_footer', 'custom_admin_order_status_scriptt');

function custom_admin_order_status_scriptt() {
    ?>
...
//hide help 3
add_action('admin_footer', 'hide_dokan_helps4_content');
function hide_dokan_helps4_content() {if (is_admin())
    ?>
<style>
.help-text {
