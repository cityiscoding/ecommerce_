...
/**
 *  Pallmall vận chuyển thường xuyên Class
 *
...
class ProductShipping extends WC_Shipping_Method {
    ...
    public function __construct() {
        ...
        $this->method_title       = __( 'PallMall Giao hàng', 'dokan' );
        $this->method_description = __( 'Cho phép các nhà cung cấp đặt chi phí vận chuyển cho mỗi sản phẩm và mỗi quốc gia', 'dokan' );
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'          => 'checkbox',
                'label'         => __( 'Cho Phép Vận Chuyển', 'dokan' ),
                'default'       => 'yes',
            ),
            'title' => array(
                'title'         => __( 'Tiêu đề phương thức', 'dokan' ),
                'type'          => 'text',
                'description'   => __( 'Giúp hiển thị tiêu đề mà người dùng nhìn thấy trong khi thanh toán.', 'dokan' ),
                'default'       => __( 'Vận chuyển A', 'dokan' ),
                'desc_tip'      => true,
            ),
            'tax_status' => array(
                'title'         => __( 'Tình trạng thuế', 'dokan' ),
                'type'          => 'select',
                'default'       => 'taxable',
                'options'       => array(
    ...
    public function calculate_per_seller( $products, $destination_country, $destination_state ) {
        ...
        if ( $seller_products ) {
            foreach ( $seller_products as $seller_id => $products ) {
                ...
                } elseif ( ! array_key_exists( $destination_country, $dps_country_rates ) ) {
