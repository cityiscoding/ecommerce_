...
class Ajax {
    ...
    public function load_variations() {
        ...
        $backorder_options = [
            'no'     => __( 'Không cho phép', 'dokan' ),
            'notify' => __( 'Cho phép, nhưng thông báo cho khách hàng', 'dokan' ),
            'yes'    => __( 'Cho phép', 'dokan' ),
        ];

        // set stock status options
        $stock_status_options = [
            'instock'     => __( 'Còn hàng', 'dokan' ),
            'outofstock'  => __( 'Hết hàng', 'dokan' ),
            'onbackorder' => __( 'Đang chờ hàng về kho', 'dokan' ),
        ];
...
<tr>
  <td>
    ...
    <select name="dps_state_to[<?php echo $country_id; ?>][]" class="dokan-form-control dps_state_selection"
      id="dps_state_selection">
  ...
  <td>
    ...
    <div class="dokan-input-group">
      ...
      <input type="text" placeholder="0.00" class="dokan-form-control"
        name="dps_state_to_price[<?php echo $country_id; ?>][]">
    </div>
  </td>
  <td width="15%">
    <label for=""></label>
...
<tr>
  <td>
    <label for=""><?php _e( 'State', 'dokan' ); ?></label>
    <input type="text" name="dps_state_to[<?php echo $country_id; ?>][]" class="dokan-form-control dps_state_selection"
...
<tr class="dokan-<?php echo $single; ?>attribute-options">
  <td width="20%">
    <input type="text" disabled="disabled" value="<?php echo $attr_name; ?>"
      class="dokan-form-control dokan-<?php echo $single; ?>attribute-option-name-label"
      data-attribute_name="<?php echo wc_sanitize_taxonomy_name( str_replace( 'pa_', '', $attribute_taxonomy_name ) ); ?>">
    <input type="hidden" name="attribute_names[]" value="<?php echo esc_attr( $attribute_taxonomy_name ); ?>"
      class="dokan-<?php echo $single; ?>attribute-option-name">
    <input type="hidden" name="attribute_is_taxonomy[]" value="1">
  </td>
  <td colspan="3"><input type="text" name="attribute_values[]" value="<?php echo implode( ',', $att_val ); ?>"
      data-preset_attr="<?php echo implode( ',', $att_val ); ?>"
      class="dokan-form-control dokan-<?php echo $single; ?>attribute-option-values"></td>
  <td>
    <button title="<?php _e( 'Clear All', 'dokan' ); ?>"
      class="dokan-btn dokan-btn-theme clear_attributes"><?php _e( 'Clear', 'dokan' ); ?></button>
    <button title="Delete" class="dokan-btn dokan-btn-theme remove_<?php echo $remove_btn; ?>attribute"><i
      ...
      <ul class="list-unstyled ">
        <li>
          <label class="checkbox-inline">
            ...
            <?php _e( 'Visible on the product page', 'dokan' ); ?>
          </label>
        </li>
        <li class="enable_variation" <?php echo ( $_POST['type'] === 'simple' ) ? 'style="display:none;"' : ''; ?>>
          <label class="checkbox-inline">
            ...
            <?php _e( 'Used for variations', 'dokan' ); ?></label>
        </li>
      </ul>
    </div>
    <div class="attribute-options">
      <ul class="option-couplet list-unstyled ">
        <?php
                        if ( $options ) {
                            foreach ( $options as $count => $option ) {
                                ?>
        <li>
          <input type="text" class="option" placeholder="<?php _e( 'Option...', 'dokan' ); ?>"
            name="attribute_values[<?php echo $i; ?>][<?php echo $count; ?>]"
        ...
        <?php
                            ...
                            ?>
        <li>
          <input type="text" class="option" name="attribute_values[<?php echo $i; ?>][0]"
            placeholder="<?php _e( 'Option...', 'dokan' ); ?>">
          <span class="item-action actions">
            <a href="#" class="row-add">+</a>
            <a href="#" class="row-remove">-</a>
          </span>
