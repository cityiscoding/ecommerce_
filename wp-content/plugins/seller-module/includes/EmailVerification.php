...
class EmailVerification {
    ...
    public function check_verification( $redirect, $user = array() ) {
        ...
        wc_add_notice( sprintf( '%s %s <a href="%s">%s</a>', $notice, __( 'Không nhận được Email?', 'dokan' ), $link, __( 'Ấn để gửi lại', 'dokan' ) ) );
    ...
    public function add_activation_link( $email ) {
        ...
        $message = sprintf( __( "<p><b>Để xác minh tài khoản trên cửa hàng của bạn <a href='%s'>Bấm vào đây</a></b></p>", 'dokan' ), $verification_link );
    ...
    public function add_email_verification_field( $user ) {
        ...
        ?>
<tr>
  <th><?php esc_html_e( 'Xác minh nhà cung cấp', 'dokan' ); ?></th>
  <td>
    <label for="verify_vendor">
      <input name="verify_vendor" type="checkbox" id="verify_vendor" value="yes"
        <?php checked( $verified_vendor, 'yes' ); ?> />
      <?php esc_html_e( 'Làm cho nhà cung cấp này được xác minh.', 'dokan' ); ?>
    </label>
  </td>
</tr>
<?php
    ...
    public function show_resend_email_notification() {
        ...
        wc_add_notice( __( 'Một email xác minh mới đã được gửi đến email của bạn.Vui lòng xác minh để đăng nhập.', 'dokan' ) );
    }
    ...
    public function dokan_email_verification_settings( $sections ) {
        $sections[] = [
            ...
            'title'                => __( 'Email xác thực', 'dokan' ),
            'icon_url'             => DOKAN_PRO_PLUGIN_ASSEST . '/images/admin-settings-icons/email-verify.svg',
            'description'          => __( 'Quản lý email xác minh', 'dokan' ),
            'settings_title'       => __( 'Cài đặt xác minh email', 'dokan' ),
            'settings_description' => __( 'Bạn có thể thiết lập xác thực hai bước cho các địa chỉ email được sử dụng trong việc tạo tài khoản.', 'dokan' ),
        ];
        return $sections;
    }
    ...
    public function dokan_email_settings_fields( $settings_fields ) {
        $settings_fields['dokan_email_verification'] = [
            'enabled' => [
                'name'    => 'enabled',
                'label'   => __( 'Bật xác minh email', 'dokan' ),
                'type'    => 'switcher',
                'desc'    => __( 'Kích hoạt sẽ gửi một email xác minh để cho phép người dùng xác minh địa chỉ email của họ sau khi mẫu đăng ký được gửi.', 'dokan' ),
                'tooltip' => __( 'Kiểm tra nếu muốn người dùng xác minh địa chỉ email của họ khi đăng ký.', 'dokan' ),
            ],
            'registration_notice' => [
                'name'  => 'registration_notice',
                'label' => __( 'Thông báo đăng ký', 'dokan' ),
                'type'  => 'textarea',
                'rows'  => 4,
                'desc'  => __( 'Thông báo này sẽ được hiển thị sau khi người dùng đã đăng ký xác minh email đang chờ xử lý.', 'dokan' ),
                'default' => __( 'Vui lòng kiểm tra email của bạn và hoàn thành xác minh email để đăng nhập.', 'dokan' ),
            ],
            'login_notice' => [
                'name'    => 'login_notice',
                'label'   => __( 'Thông báo đăng nhập', 'dokan' ),
                'type'    => 'textarea',
                'rows'    => 4,
                'desc'    => __( 'Thông báo này sẽ được hiển thị khi người dùng cố gắng đăng nhập mà không cần xác minh email.', 'dokan' ),
                'default' => __( 'Vui lòng kiểm tra email của bạn và hoàn thành xác minh email để đăng nhập.', 'dokan' ),
            ],
        ];
    ...
    public function double_opt_in_admin_notice( $notices ) {
        ...
        $email_verification_option_url = sprintf(
            ...
            esc_attr__( 'PallMall Email xác thực', 'dokan' ),
            esc_attr__( 'Email xác thực', 'dokan' )
        );
