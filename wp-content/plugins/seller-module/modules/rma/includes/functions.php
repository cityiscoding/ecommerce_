...
function dokan_rma_warranty_type( $type = '' ) {
    $warranty_type = apply_filters( 'dokan_rma_warranty_type', [
        'no_warranty'       => __( 'Không bảo hành', 'dokan' ),
...
function dokan_rma_warranty_length( $length = '' ) {
    $warranty_length = apply_filters( 'dokan_rma_warranty_length', [
        'limited'  => __( 'Giới hạn', 'dokan' ),
        'lifetime' => __( 'Cả đời', 'dokan' )
    ] );
...
function dokan_get_warranty_duration_string( $warranty, $order ) {
    ...
    if ( $warranty['type'] == 'no_warranty' ) {
        $warranty_string = __( 'Sản phẩm không được bảo hành và trả hàng', 'dokan' );
...
function dokan_warranty_request_type( $type = '' ) {
    $types = apply_filters( 'dokan_warranty_request_type', [
        'replace' => __( 'Trả hàng | Hoàn tiền', 'dokan' ),
        'refund'  => __( 'Bảo hành', 'dokan' ),
        'coupon'  => __( 'Hoàn lại tiền dưới dạng tín dụng cửa hàng', 'dokan' )
    ] );
...
function dokan_save_warranty_request( $data = [] ) {
    ...
    if ( empty( $data['order_id'] ) ) {
        return new WP_Error( 'no-order-id', __( 'Không tìm thấy đơn đặt hàng', 'dokan' ) );
    }

    // Check if not have any vendor
    if ( empty( $data['vendor_id'] ) ) {
        return new WP_Error( 'no-vendor-id', __( 'Không tìm thấy nhà cung cấp', 'dokan' ) );
    }

    // Check if customer select any product or not. If not select any product we do not proceed to create request
    if ( empty( $data['items'] ) ) {
        return new WP_Error( 'no-product-id', __( 'Vui lòng chọn một số mục để gửi yêu cầu', 'dokan' ) );
    }

    if ( empty( $data['type'] ) ) {
        return new WP_Error( 'no-type', __( 'Loại yêu cầu là bắt buộc', 'dokan' ) );
    }
...
function dokan_update_warranty_request( $data = [] ) {
    ...
    if ( empty( $data['id'] ) ) {
        return new WP_Error( 'no-request-id', __( 'Không tìm thấy ID yêu cầu', 'dokan' ) );
    }
    ...
    if ( ! in_array( $data['status'], array_keys( $statuses ) ) ) {
        return new WP_Error( 'no-valid-status', __( 'Trạng thái của bạn không hợp lệ', 'dokan' ) );
    }

    $result = $wpdb->update( $request_table, $data, [ 'id' => $data['id'] ] );

    if ( ! $result ) {
        return new WP_Error( 'status-not-updated', __( 'Trạng thái không được cập nhật, vui lòng thử lại', 'dokan' ) );
    }
...
<ul class="list-inline request-statuses-filter">
  <li<?php echo $status_class == 'all' ? ' class="active"' : ''; ?>>
    ...
    <li<?php echo $status_key == $status_class ? ' class="active"' : ''; ?>>
      <a
        href="<?php echo add_query_arg( array( 'status' => $status_key ), $permalink ); ?>"><?php printf( __( '%s (%d)', 'dokan' ), $statuses[$status_key], $count ); ?></a>
      </li>
      <?php
            }
        }
