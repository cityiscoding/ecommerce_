...
class Helper {
	...
	public static function get_dokan_seller_badge_events( $event = '', $get_event_keys = false ) {
		...
		$events = [
			'product_published'     => [
				...
				'description'         => __( 'Khi một nhà cung cấp xuất bản một số sản phẩm nhất định', 'dokan' ),
				'condition_text'      => [
					'prefix' => __( 'Khi nhà cung cấp xuất bản', 'dokan' ),
			...
			'years_active'          => [
				...
				'condition_text'      => [
					'prefix' => __( 'Khi một nhà cung cấp bán trong nền tảng này trong hơn một năm', 'dokan' ),
					'suffix' => '',
				],
			...
			'sale_amount'           => [
				...
				'hover_text'          => '{badge_name}: Đã bán nhiều hơn {badge_data_price}',
				'group'               => [
					'key'   => 'amount',
	...
	public static function get_formatted_event_status( $status = '' ) {
		$event_status = [
			'published' => __( 'Công khai', 'dokan' ),
			'draft'     => __( 'Bản nháp', 'dokan' ),
		];
	...
	public static function get_condition_data( $condition ) {
		switch ( $condition ) {
			case '>':
				$condition = __( 'Nhiều hơn', 'dokan' );
				break;
			case '<':
				$condition = __( 'Ít hơn', 'dokan' );
				break;
			case '=':
				$condition = __( 'Tương đương với', 'dokan' );
				break;
			default:
				$condition = '';
				break;
