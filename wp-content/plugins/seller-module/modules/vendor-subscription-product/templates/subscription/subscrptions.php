    ...
    <article class="dashboard-user-subscription-area">
      ...
      <form action="" method="GET" class="dokan-left">
        <div class="dokan-form-group">
          <select name="customer_id" id="dokan-filter-customer" class="dokan-form-control dokan-w12"
            data-allow_clear="true" data-placeholder="<?php esc_attr_e( 'Filter by Registered Customer', 'dokan' ); ?>">
            <option value="<?php echo esc_attr( $customer_id ); ?>" selected="selected">
              <?php echo esc_html( $user_string ); ?></option>
          </select>
          <input type="text" class="datepicker" style="width:120px; padding-bottom:7px" name="order_date"
            id="order_date_filter" placeholder="<?php esc_attr_e( 'Filter by Date', 'dokan' ); ?>"
            value="<?php echo esc_attr( $filter_date ); ?>">
          <input type="submit" name="dokan_order_filter" class="dokan-btn dokan-btn-sm"
            value="<?php esc_attr_e( 'Filter', 'dokan' ); ?>">
      ...
      <table class="dokan-table">
        ...
        <tbody>
          ...
          <tr>
            <td><label
                class="dokan-label dokan-label-<?php echo esc_attr( dokan_vps_get_subscription_status_class( $subscription->get_status() ) ); ?>"><?php echo esc_html( dokan_vps_get_subscription_status_translated( $subscription->get_status() ) ); ?></label>
            </td>
            <td>
              <?php if ( current_user_can( 'dokan_view_order' ) ) : ?>
              <a
                href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'subscription_id' => $subscription->get_id() ), dokan_get_navigation_url( 'user-subscription' ) ) ) ); ?>">
                <strong>
                  <?php
            ...
            <td>
              <?php
                                ...
                                $price_content .= esc_html( sprintf( __( 'Thông qua %s', 'dokan' ), $subscription->get_payment_method_to_display() ) );
