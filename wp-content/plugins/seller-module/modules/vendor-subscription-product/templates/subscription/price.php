...
<div class="dokan-clearfix dokan-price-container dokan-subscription-product-price show_if_subscription">
  ...
  <div class="dokan-form-group subscription-price dokan-clearfix">
    <label for="_subscription_price" class="form-label"><?php esc_html_e( 'Subscription price', 'dokan' ); ?></label>
    ...
    <div class="dokan-input-group">
      <select id="_subscription_period_interval" name="_subscription_period_interval" class="dokan-form-control">
        ...
        <option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $chosen_interval, true ) ?>>
    ...
    <div class="dokan-input-group">
      <select id="_subscription_period" name="_subscription_period" class="dokan-form-control">
        ...
        <option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $chosen_period, true ) ?>>
          <?php echo esc_html( $label ); ?></option>
        <?php } ?>
      </select>
    </div>
  ...
  <div class="content-full-part sale-price dokan-clearfix dokan-form-group">
    <label for="_subscription_sale_price" class="form-label">
      ...
      <a href="#"
        class="sale_schedule <?php echo ($show_schedule ) ? 'dokan-hide' : ''; ?>"><?php esc_html_e( 'Schedule', 'dokan' ); ?></a>
      <a href="#"
        class="cancel_sale_schedule <?php echo ( ! $show_schedule ) ? 'dokan-hide' : ''; ?>"><?php esc_html_e( 'Cancel', 'dokan' ); ?></a>
    </label>
  ...
  <div
    ...
    <div class="content-half-part from">
      <div class="dokan-input-group">
        ...
        <input type="text" name="_subscription_sale_price_dates_from" class="dokan-form-control dokan-start-date"
          value="<?php echo esc_attr( $_sale_price_dates_from ); ?>" maxlength="10"
          pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])"
          placeholder="<?php esc_html_e( 'YYYY-MM-DD', 'dokan' ); ?>">
      </div>
    </div>
    ...
    <div class="content-half-part to">
      <div class="dokan-input-group">
        ...
        <input type="text" name="_subscription_sale_price_dates_to" class="dokan-form-control dokan-end-date"
  ...
  <div class="dokan-form-group subscription-expire">
    <label for="_subscription_length" class="form-label"><?php esc_html_e( 'Subscription expire after', 'dokan' ); ?>
      <select id="_subscription_length" name="_subscription_length" class="dokan-form-control">
        ...
        <option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $chosen_length, true ) ?>>
          <?php echo esc_html( $label ); ?></option>
        <?php } ?>
      </select>
  </div>
  ...
  <div class="dokan-form-group subscription-trial-length-field dokan-clearfix">
    ...
    <div class="content-half-part">
      <input type="number" step="1" min="0" name="_subscription_trial_length" id="_subscription_trial_length"
        class="dokan-form-control" value="<?php echo $chosen_trial_length ?>" placeholder="0">
    </div>
    <div class="content-half-part">
      <select id="_subscription_trial_period" name="_subscription_trial_period"
        class="dokan-form-control wc_input_subscription_trial_period last">
        <?php foreach ( wcs_get_available_time_periods() as $value => $label ) { ?>
        <option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $chosen_trial_period, true ) ?>>
          <?php echo esc_html( $label ); ?></option>
        <?php } ?>
      </select>
    </div>
  ...
  <div class="subscription-sync">
    <div class="dokan-form-group subscription_sync_week_month"
      style="<?php echo esc_attr( $display_week_month_select ) ?>">
      <label for="<?php echo WC_Subscriptions_Synchroniser::$post_meta_key;?>"
        class="form-label"><?php esc_html_e( 'Synchronise renewals', 'dokan' ); ?></label>
      <select id="<?php echo WC_Subscriptions_Synchroniser::$post_meta_key;?>"
        name="<?php echo WC_Subscriptions_Synchroniser::$post_meta_key;?>"
        class="dokan-form-control wc_input_subscription_payment_sync select">
        <?php foreach ( WC_Subscriptions_Synchroniser::get_billing_period_ranges( $subscription_period ) as $value => $label ) { ?>
        <option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $payment_day, true ) ?>>
          <?php echo esc_html( $label ); ?></option>
        <?php } ?>
      </select>
    </div>
    <div class="dokan-form-group subscription_sync_annual" style="<?php echo esc_attr( $display_annual_select ) ?>">
      <label for="<?php echo WC_Subscriptions_Synchroniser::$post_meta_key_month;?>"
        class="form-label"><?php esc_html_e( 'Synchronise renewals', 'dokan' ); ?></label>
      <div class="dokan-form-group dokan-clearfix">
        <div class="content-half-part">
          <input type="number" id="<?php echo esc_attr( WC_Subscriptions_Synchroniser::$post_meta_key_day ); ?>"
            name="<?php echo esc_attr( WC_Subscriptions_Synchroniser::$post_meta_key_day ); ?>"
            class="dokan-form-control wc_input_subscription_payment_sync"
            value="<?php echo esc_attr( $payment_day ); ?>" placeholder="<?php esc_html_e( 'Day', 'dokan' );?>" />
        </div>
        <div class="content-half-part">
          <select id="<?php echo WC_Subscriptions_Synchroniser::$post_meta_key_month;?>"
            name="<?php echo WC_Subscriptions_Synchroniser::$post_meta_key_month;?>"
            class="dokan-form-control wc_input_subscription_payment_sync">
            <?php foreach ( $wp_locale->month as $value => $label ) { ?>
            <option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $payment_month, true ) ?>>
              <?php echo esc_html( $label ); ?></option>
            <?php } ?>
          </select>
        </div>
