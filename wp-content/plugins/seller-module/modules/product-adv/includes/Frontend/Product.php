...
class Product {
    ...
    public function render_advertise_product_section( $post_id ) {
        ...
        dokan_get_template_part(
            'product-advertisement-content', '', array_merge(
                [
                    'product_id'               => $post_id,
                    'advertise_active_color'   => ! empty( $colors['btn_primary'] ) ? $colors['btn_primary'] : 'tomato',
    ...
    public function product_listing_table_content( $post, $product ) {
        ...
        if ( $advertisement_data['already_advertised'] ) {
            ...
            $colors = dokan_get_option( 'store_color_pallete', 'dokan_colors', [] );
            $title  = sprintf( __( 'Hết hạn vào: %s', 'dokan' ), $advertisement_data['expire_date'] );
            $color  = ! empty( $colors['btn_primary'] ) ? $colors['btn_primary'] : 'tomato';
            $class  = 'advertised';
        }
...
EOD;
    ...
    public function product_listing_table_column() {
        $colors = dokan_get_option( 'store_color_pallete', 'dokan_colors', [] );
        $color  = ! empty( $colors['btn_primary'] ) ? $colors['btn_primary'] : 'tomato';
        $title  = __( 'Sản phẩm được quảng cáo', 'dokan' );
        echo <<<EOD
...
EOD;
    ...
    public function load_product_scripts() {
        ...
        // localize scripts
        $localized_data = [
            'advertise_alert'              => esc_html__( 'Bạn có chắc là bạn muốn quảng cáo sản phẩm này?', 'dokan' ),
            ...
            'on_error_message'             => esc_html__( 'Đã xảy ra lỗi.', 'dokan' ),
            'on_success_message'           => esc_html__( 'Thành công.', 'dokan' ),
            'product_not_published'        => esc_html__( 'Bạn không thể quảng cáo sản phẩm này.Sản phẩm cần được xuất bản trước khi bạn có thể quảng cáo.', 'dokan' ),
            'on_load_advertisement_status' => esc_html__( 'Tải dữ liệu quảng cáo.Vui lòng chờ...', 'dokan' ),
