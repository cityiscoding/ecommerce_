...
class Helper {
    ...
    public static function get_advertisement_data_for_insert( $product_id, $vendor_id ) {
        ...
        if ( empty( $advertisement_data ) ) {
            return new WP_Error( 'invalid_product', __( 'Không có sản phẩm được tìm thấy với ID sản phẩm đã cho.Vui lòng kiểm tra đầu vào của bạn.', 'dokan' ) );
        }

        // check if product status is publish
        if ( 'publish' !== $advertisement_data['post_status'] ) {
            return new WP_Error( 'invalid_product', __( 'Bạn không thể quảng cáo sản phẩm này.Sản phẩm cần được xuất bản trước khi bạn có thể quảng cáo.', 'dokan' ) );
        }
        ...
        if ( ! $advertisement_data['vendor_id'] || intval( $vendor_id ) !== $advertisement_data['vendor_id'] ) {
            return new WP_Error( 'invalid_vendor', __( 'ID sản phẩm không thuộc về nhà cung cấp đã cho.Vui lòng kiểm tra đầu vào của bạn', 'dokan' ) );
        }
        ...
        if ( $advertisement_data['already_advertised'] ) {
            return new WP_Error( 'invalid_product', __( 'Quảng cáo cho sản phẩm này đã có.Vui lòng chọn một sản phẩm khác.', 'dokan' ) );
        }

        // check we've got slot left for advertisement
        if ( empty( $advertisement_data['remaining_slot'] ) ) {
            return new WP_Error( 'empty_slot', __( 'Không có khe quảng cáo có sẵn tại thời điểm này.', 'dokan' ) );
        }
    ...
    public static function create_advertisement_base_product() {
        ...
        // create a new post
        $post = [
            'post_content' => 'Đây là sản phẩm thanh toán quảng cáo của PallMall, không thể xóa.',
            'post_status'  => 'publish',
    ...
    public static function purchase_advertisement( $product_id ) {
        try {
            ...
            if ( ! current_user_can( 'dokandar' ) ) {
                throw new Exception( __( 'Bạn không có quyền sử dụng hành động này.', 'dokan' ), 400 );
            }
            ...
            if ( ! Helper::is_per_product_advertisement_enabled() && ! Helper::is_enabled_for_vendor_subscription() ) {
                throw new Exception( __( 'Mua quảng cáo bị hạn chế bởi quản trị viên.', 'dokan' ), 403 );
            }
            ...
            if ( false !== $advertisement_data['can_advertise_for_free'] ) {
                ...
                return [
                    'message'       => __( 'Sản phẩm đã được quảng cáo thành công.', 'dokan' ),
                    'free_purchase' => true,
                ];
            }
            ...
            if ( ! is_numeric( $advertisement_product_id ) ) {
                throw new Exception( __( 'ID sản phẩm quảng cáo không hợp lệ.Vui lòng liên hệ với quản trị viên trang web.', 'dokan' ), 400 );
            }

            $advertisement_product = wc_get_product( $advertisement_product_id );
            if ( ! $advertisement_product ) {
                throw new Exception( __( 'Sản phẩm quảng cáo không hợp lệ được tìm thấy.Vui lòng liên hệ với quản trị viên trang web.', 'dokan' ), 400 );
            }

            if ( $advertisement_product->get_status() !== 'publish' ) {
                throw new Exception( __( 'Tình trạng sản phẩm quảng cáokhông được công bố.Vui lòng liên hệ với quản trị viên trang web.', 'dokan' ), 400 );
            }
            ...
            if ( $added ) {
                return [
                    'message'       => __( 'Sản phẩm đã được thêm vào giỏ hàng của bạn. Mã giảm giá: ADPRODUCT30. Giúp bạn giảm 30% khi thanh toán "Quảng bá sản phẩm". Nhấn OK để tiến thành thanh toán ngay!', 'dokan' ),
                    'free_purchase' => false,
                ];
            }

            throw new Exception( __( 'Đã xảy ra lỗi bất định.', 'dokan' ), 400 );
        } catch ( Exception $e ) {
