...
<div id="dokan-delivery-time-box">
  <div class="header">
    ...
    <div class="delivery-timezone dokan-delivery-time-tooltip"
      data-tip="<?php esc_attr_e( 'Date & time is based on current site time.', 'dokan' ); ?>" tabindex="1">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" height="18" viewBox="0 0 24 24" stroke="#333333">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.85"
          d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
      <span
        class=""><?php echo esc_html( dokan_format_datetime() ); ?><?php echo wc_timezone_string() ? ', ' . wc_timezone_string() : ''; ?></span>
    </div>
  </div>
  <div class="delivery-time-body">
    ...
    <div class="delivery-group">
      <input type="hidden" value="<?php echo esc_attr( $id ); ?>"
        name="<?php echo 'vendor_delivery_time[' . $id . '][vendor_id]'; ?>">
      <input type="hidden" value="<?php echo esc_attr( $info['store_name'] ); ?>"
        name="<?php echo 'vendor_delivery_time[' . $id . '][store_name]'; ?>">
      <div class="title">
      ...
      <?php
                    ...
                    $delivery_date_label = isset( $info['vendor_delivery_options']['delivery_date_label'] ) ? $info['vendor_delivery_options']['delivery_date_label'] : __( 'Lựa chọn ngày', 'dokan' );
                    ?>

      <input id="delivery-time-date-picker-<?php echo esc_attr( $id ); ?>"
        data-vendor_id="<?php echo esc_attr( $id ); ?>"
        data-nonce="<?php echo esc_attr( wp_create_nonce( 'dokan_delivery_time' ) ); ?>"
        class="delivery-time-date-picker" name="vendor_delivery_time[<?php echo esc_attr( $id ); ?>][delivery_date]"
        type="text" placeholder="<?php echo esc_attr( $delivery_date_label ); ?>" readonly="readonly">
      <?php
                    /**
                     * @since 3.3.7
                     *
      ...
      <input class="dokan-selected-delivery-type" id="dokan-selected-delivery-type-<?php echo esc_attr( $id ); ?>"
        name="vendor_delivery_time[<?php echo esc_attr( $id ); ?>][selected_delivery_type]" type="hidden"
        data-vendor_id="<?php echo esc_attr( $id ); ?>" value="<?php echo esc_attr( $selected_delivery_type ); ?>">

      <select class="delivery-time-slot-picker" id="delivery-time-slot-picker-<?php echo esc_attr( $id ); ?>"
        name="vendor_delivery_time[<?php echo esc_attr( $id ); ?>][delivery_time_slot]">
