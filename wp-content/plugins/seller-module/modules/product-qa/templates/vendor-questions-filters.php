...
<form action='' method='get' class='dokan-form-inline dokan-w12 dokan-product-qa-filter-form'>
  ...
  <div class='dokan-form-group dokan-product-qa-product-search-form-group'>
    <select name='product_id' class='dokan-form-control dokan-product-search dokan-product-qa-product-search'
      data-placeholder="<?php esc_attr_e( 'Tìm kiếm một sản phẩm&hellip;', 'dokan' ); ?>"
      ...
      <option value="<?php echo esc_attr( $product_id ); ?>" selected="selected">
        <?php echo esc_html( $product_title ); ?></option>
      <?php else : ?>
  ...
  <input type="submit" value="<?php esc_attr_e( 'Lọc', 'dokan' ); ?>" class="dokan-btn">
  <a class="dokan-btn"
    href="<?php echo esc_url( dokan_get_navigation_url( Vendor::QUERY_VAR ) ); ?>"><?php esc_attr_e( 'Cài lại (reset)', 'dokan' ); ?>
  </a>
