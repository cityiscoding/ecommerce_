    ...
    public function enqueue_scripts() {
        ...
        if( isset( $wp->query_vars['settings'] ) && 'store' === $wp->query_vars['settings'] ) {
            ...
            wp_localize_script( 'dokan-seller-vacation', 'dokanSellerVacation', array(
                'i18n' => array(
                    'vacation_date_is_not_set' => __( 'Không có kỳ nghỉ được thiết lập', 'dokan' ),
                    'invalid_from_date'        => __( 'Ngày bắt đầu không hợp lệ', 'dokan' ),
                    'invalid_to_date'          => __( 'Ngày hết hạn không hợp lệ', 'dokan' ),
                    'empty_message'            => __( 'Tin nhắn trống', 'dokan' ),
                    'save'                     => __( 'Lưu thiết lập', 'dokan' ),
                    'saving'                   => __( 'Đang lưu', 'dokan' ),
                    'confirm_delete'           => __( 'Bạn có chắc bạn muốn xóa mục này?', 'dokan' ),
                ),
            ) );
        }
    }
