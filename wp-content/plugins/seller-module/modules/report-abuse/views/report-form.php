<form class="dokan-form-container" id="<?php echo esc_attr( $id ); ?>">
  ...
  <fieldset>
    <ul class="<?php echo $option_list_classes; ?>">
      ...
      <li>
        <label class="<?php echo $option_label_classes; ?>">
          <input required type="radio" name="reason" value="<?php echo esc_attr( $abuse_reason['value'] ); ?>">
          <?php echo esc_html( $abuse_reason['value'] ); ?>
        </label>
      </li>
      <?php endforeach; ?>
    </ul>
    ...
    <div class="dokan-form-group">
      ...
      <button type="button" class="dokan-w4 dokan-btn dokan-btn-theme dokan-hide"
        id="dokan-report-abuse-form-working-btn">
        <i class="fas fa-sync-alt fa-spin"></i>&nbsp;&nbsp;<?php esc_html_e( 'Đang gửi', 'dokan-lite' ); ?>...
      </button>
    </div>
  </fieldset>
</form>
