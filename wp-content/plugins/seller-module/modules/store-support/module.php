...
class Module {
    ...
    public function render_store_support_section( $sections ) {
        $sections[] = [
            ...
            'title'                => __( 'Hỗ trợ cửa hàng', 'dokan' ),
            ...
            'settings_title'       => __( 'Hỗ trợ cửa hàng Settings', 'dokan' ),
    ...
    public function enqueue_scripts() {
        ...
        if (
            ...
            wp_localize_script(
                ...
                [
                    'wait'              => __( 'wait...', 'dokan' ),
                    'empty_comment_msg' => __( 'Bạn chưa nhập câu trả lời :(', 'dokan' ),
                ]
            );
        }
...
<li class="dokan-store-support-btn-wrap dokan-right">
  <button data-store_id="<?php echo esc_attr( $store_id ); ?>"
    class="dokan-store-support-btn dokan-btn dokan-btn-theme dokan-btn-sm <?php echo esc_attr( $button['class'] ); ?>"><?php echo esc_html( $button['text'] ); ?></button>
</li>
<?php
    }
    ...
    public function generate_support_button_product_page() {
        ...
        ?>
<button data-store_id="<?php echo esc_attr( $store_id ); ?>"
  class="dokan-store-support-btn-product dokan-store-support-btn button alt <?php echo esc_attr( $button['class'] ); ?>"><?php echo esc_html( $button['text'] ); ?></button>
<?php
    }
    ...
    public function generate_support_button_product_page_inner_tab( $author, $store ) {
        ...
        ?>
<button data-store_id="<?php echo esc_attr( $author->ID ); ?>"
  class="dokan-store-support-btn-product dokan-store-support-btn button alt <?php echo esc_attr( $button['class'] ); ?>"><?php echo esc_html( $button['text'] ); ?></button>
<?php
    }
    ...
    public function generate_support_button_customer_order_page( $order ) {
        ...
        ?>
<button data-order_id="<?php echo esc_attr( $order_id ); ?>" data-store_id="<?php echo esc_attr( $store_id ); ?>"
  class="dokan-store-support-btn-product dokan-store-support-btn button alt <?php echo esc_attr( $button['class'] ); ?>"><?php echo esc_html( $button['text'] ); ?></button>
<?php
    }
...
<h2><?php esc_html_e( 'Vui lòng đăng nhập để tiếp tục', 'dokan' ); ?></h2>

<form class="dokan-form-container" id="dokan-support-login">
  <div class="dokan-form-group">
    <label class="dokan-form-label" for="login-name"><?php esc_html_e( 'Tên đăng nhập hoặc Email', 'dokan' ); ?></label>
    <input required class="dokan-form-control" type="text" name='login-name' id='login-name' />
  </div>
  <div class="dokan-form-group">
    <label class="dokan-form-label" for="login-password"><?php esc_html_e( 'Mật khẩu :', 'dokan' ); ?></label>
  ...
  <div class="dokan-form-group">
    <input id='support-submit-btn' type="submit" value="<?php esc_html_e( 'Đăng nhập', 'dokan' ); ?>"
      class="dokan-w5 dokan-btn dokan-btn-theme" />
  </div>
  <p class="dokan-popup-create-an-account">
    &nbsp;&nbsp; <?php esc_html_e( 'hoặc', 'dokan' ); ?> &nbsp;&nbsp; <a href="<?php echo esc_url( $login_url ); ?>"
      class="dokan-btn dokan-btn-theme"><?php esc_html_e( 'Tạo tài khoản mới', 'dokan' ); ?></a>
  </p>
</form>
...
<?php
    ...
    public function get_support_form( $seller_id = '' ) {
        ...
        ?>
<div class="dokan-support-intro-user">
...
<form class="dokan-form-container" id="dokan-support-form">
  ...
  <div class="dokan-form-group">
    ...
    <select class="dokan-form-control dokan-select" name="order_id">
      <option><?php esc_html_e( 'Lựa chọn id của đơn hàng muốn yêu cầu hỗ trợ', 'dokan' ); ?></option>
      <?php
                        foreach ( $customer_orders as $order ) :
                            $selected = $order_id && ( $order === $order_id ) ? 'selected' : '';
                            ?>
      <option value='<?php echo esc_attr( $order ); ?>' <?php echo esc_attr( $selected ); ?>>
        <?php esc_html_e( 'Order', 'dokan' ); ?> #<?php echo esc_html( $order ); ?></option>
      <?php endforeach; ?>
    </select>
    <?php } ?>
  </div>
  ...
  <div class="dokan-form-group">
    <input id='support-submit-btn' type="submit" value="<?php esc_html_e( 'Submit', 'dokan' ); ?>"
      class="dokan-w5 dokan-btn dokan-btn-theme" />
  </div>
</form>
<div class="dokan-clearfix"></div>
<?php
    ...
    public function support_msg_submit( $postdata = [] ) {
        ...
        if ( $post_id ) {
            ...
            $success_msg = __( 'Cảm ơn bạn.Yêu cầu hỗ của bạn đã được gửi!', 'dokan' );
    ...
    <tbody>
      ...
      <tr>
        ...
        <?php
                        ...
                        ?>
        <td data-title="<?php esc_attr_e( 'Status', 'dokan' ); ?>"><span
            class="dokan-label <?php echo esc_attr( $topic_status ); ?>"><?php echo esc_html( $status_label ); ?></span>
        </td>
        <td class="dokan-order-date" data-title="<?php esc_attr_e( 'Date', 'dokan' ); ?>">
          <span><?php echo esc_html( dokan_format_datetime( dokan_get_timestamp( $post->post_date_gmt, true ) ) ); ?></span>
        </td>
        <td data-title="<?php esc_attr_e( 'Action', 'dokan' ); ?>">
          <?php $selected_icon = 'fa-file' === $btn_icon ? esc_attr( 'far' ) : esc_attr( 'fas' ); ?>
          <a class="dokan-btn dokan-btn-default dokan-btn-sm tips dokan-support-status-change"
            onclick="dokan_show_delete_prompt( event, dokan.delete_confirm );"
            href="<?php echo esc_url( $action_url ); ?>" title="" data-changing_post_id="<?php echo get_the_ID(); ?>"
            data-original-title="<?php echo esc_attr( $btn_title ); ?>"><i
              data-url="<?php echo esc_url( $action_url ); ?>"
              class="<?php echo esc_attr( $selected_icon . ' ' . $btn_icon ); ?>">&nbsp;</i></a>
        </td>
      </tr>
      <?php
                }
...
<div class="dokan-support-single-title">
  <div class="dokan-dss-chat-header">
    ...
    <div class="dokan-chat-status-box">
      <span
        class="dokan-chat-status <?php echo get_post_status( get_the_ID() ) === 'open' ? 'chat-open' : 'chat-closed'; ?>"><?php echo get_post_status( get_the_ID() ) === 'open' ? __( 'Open', 'dokan' ) : __( 'Closed', 'dokan' ); ?></span>
    </div>
  </div>
  <?php
  ...
  <span class="order-reference">
    <h3>
      ...
      <?php echo '<a href="' . esc_url( $ticket_ref_url ) . '"><strong>' . sprintf( __( 'Đơn đặt hàng được yêu cầu hỗ trợ tham chiếu đến #%s', 'dokan' ), esc_attr( number_format_i18n( $order_id ) ) ) . '</strong></a>'; ?>
    </h3>
  </span>
  <?php
                    }
...
<div class="dokan-panel dokan-panel-default dokan-dss-panel-default">
  <div class="dokan-dss-panel-heading">
    <?php
                    $heading = $ticket_status === 'open' ? __( 'Thêm câu trả lời', 'dokan' ) : __( 'Ticked đã đóng', 'dokan' );
                    ?>
                    ...
                    if ( $ticket_status === 'open' || ! $is_customer ) {
                        ...
                        $select_topic_status = $ticket_status === 'open' ?
                                                '<select class="dokan-support-topic-select dokan-form-control dokan-w3" name="dokan-topic-status-change">
                                                    <option value="0">' . __( '-- Thay đổi trạng thái --', 'dokan' ) . '</option>
                                                    <option value="1">' . __( 'Đóng ticked', 'dokan' ) . '</option>
                                                </select>'
                        ...
                        $comment_args = [
                            ...
                            'name_submit'          => 'submit',
                            'title_reply'          => __( 'Để lại một câu trả lời', 'dokan' ),
                            'title_reply_to'       => '',
                            'cancel_reply_link'    => __( 'Hủy trả lời', 'dokan' ),
                            'label_submit'         => __( 'Gửi câu trả lời', 'dokan' ),
                            'format'               => 'html5',
...
<div class="dokan-support-topics">
  <a href="<?php echo dokan_get_page_url( 'myaccount', 'woocommerce', 'support-tickets/' ); ?>"><button
      class="dokan-btn dokan-btn-theme"><?php esc_html_e( 'View Support Tickets', 'dokan' ); ?></button></a>
</div>
<?php
    }
...
<div class="dokan-support-topics-list">
  ...
  <table class="dokan-table dokan-support-table">
    ...
    <tbody>
      ...
      <tr>
        <td>
          <a href="<?php echo $topic_url; ?>" <strong>
            <?php
                                    // translators: %s: Topic ID
        ...
        <td>
          <div class="dokan-support-customer-name">
            <?php
                                ...
                                ?>
            <strong><a href="<?php echo esc_url( $store_url ); ?>"
                target="_blank"><?php echo esc_html( $store_name ); ?></a></strong>
          </div>
        </td>
        <td>
        ...
        <td class="dokan-order-date">
          <span><?php echo esc_html( dokan_format_datetime( dokan_get_timestamp( $topic->post_date_gmt, true ) ) ); ?></span>
        </td>
      </tr>
      <?php
                }
...
<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
  ...
  <div class="dokan-dss-chat-box">
    <div class="dokan-chat-image-box">
      <div class="dokan-chat-image-container">
        <img src="<?php echo esc_attr( $user_image ); ?>" alt="<?php echo esc_attr( $user_image ); ?>"
          class="dokan-chat-image">
      </div>
    </div>
    <div class="dokan-chat-info-box">
      <div class="dokan-chat-sender-info">
    ...
    <div class="dokan-w5 dokan-text-left">
      <div class="checkbox">
        <label>
          ...
          <input type="checkbox" id="support_checkbox" name="support_checkbox" value="yes"
            <?php checked( $enable_support, 'yes' ); ?>> <?php esc_html_e( 'Show support button in store', 'dokan' ); ?>
        </label>
        <?php if ( 'dont_show' !== $store_support_product_page ) : ?>
        <label>
          <input type="hidden" name="support_checkbox_product" value="no">
          <input type="checkbox" id="support_checkbox_product" name="support_checkbox_product" value="yes"
            <?php checked( $enable_support_product, 'yes' ); ?>>
          <?php esc_html_e( 'Show support button in single product', 'dokan' ); ?>
        </label>
        <?php endif; ?>
      </div>
    </div>
  ...
  <div class="dokan-form-group support-enable-check">
    <label class="dokan-w3 dokan-control-label"
      for="dokan_support_btn_name"><?php esc_html_e( 'Support Button text', 'dokan' ); ?></label>

    <div class="dokan-w5 dokan-text-left">
      <input id="dokan_support_btn_name" value="<?php echo $support_text; ?>" name="dokan_support_btn_name"
        placeholder="<?php esc_html_e( 'Get Support', 'dokan' ); ?>" class="dokan-form-control" type="text">
    </div>
  </div>
  <?php
    }
  ...
  <ul class="dokan-support-topic-counts subsubsub">
    <li <?php echo $current_status === 'all' ? 'class = "active"' : ''; ?>>
      <a
        href="<?php echo add_query_arg( [ 'ticket_status' => 'all' ], $redir_url ); ?>"><?php echo __( 'All Tickets', 'dokan' ) . ' (' . $all . ')'; ?></a>
    </li>
    <li <?php echo $current_status === 'open' ? 'class = "active"' : ''; ?>>
      <a
        href="<?php echo add_query_arg( [ 'ticket_status' => 'open' ], $redir_url ); ?>"><?php echo __( 'Open Tickets', 'dokan' ) . ' (' . $open . ')'; ?></a>
    </li>
    <li <?php echo $current_status === 'closed' ? 'class = "active"' : ''; ?>>
      <a
        href="<?php echo add_query_arg( [ 'ticket_status' => 'closed' ], $redir_url ); ?>"><?php echo __( 'Closed Tickets', 'dokan' ) . ' (' . $closed . ')'; ?></a>
    </li>
  </ul>
  <?php
    }
