...
<tr>
  <td data-title="<?php esc_attr_e( 'Tên cửa hàng', 'dokan' ); ?>" class="column-primary">
    <div class="dokan-w3 product-image-area">
      <?php echo wp_kses_post( $product->get_image( 'thumbnail' ) ); ?>
    </div>
    <div class="dokan-w9 product-info-area">
      <a target="_blank"
        href="<?php echo esc_url( $product->get_permalink() ); ?>"><?php echo wp_kses_post( $product_title ); ?></a><br>
      <div class="product-review-comment-area">
        <i class="fa fa-star star-icon-color" aria-hidden="true"></i> <strong>(
          <?php echo esc_html( $product->get_average_rating() ); ?> )</strong>&nbsp;&nbsp;
        <i class="fa fa-comments" aria-hidden="true"></i>
        <strong><?php echo esc_html( $product->get_review_count() ); ?></strong><br>
      </div>
  ...
  <td class="action-section" data-title="<?php esc_attr_e( 'Actions', 'dokan' ); ?>">
    ...
    <button class="dokan-btn" disabled data-product="<?php echo esc_attr( $product->get_id() ); ?>">
      <?php esc_html_e( 'Already Cloned', 'dokan' ); ?>
    </button>
    <?php else : ?>
    <button class="dokan-btn dokan-spmv-clone-product" data-product="<?php echo esc_attr( $product->get_id() ); ?>">
