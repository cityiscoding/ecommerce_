...
<div class="dokan-form-group">
  <label class="dokan-w3 dokan-control-label"><?php esc_html_e( 'Giảm giá ', 'dokan' ); ?></label>
  <div class="dokan-w5 dokan-text-left">
    <div class="checkbox">
      <label class="dokan-control-label" for="lbl_setting_minimum_quantity">
        ...
        <input id="lbl_setting_minimum_quantity" type="checkbox" name="setting_show_minimum_order_discount_option"
          value="yes" <?php checked( $is_enable_order_discount, 'yes' ); ?> />
        <?php esc_html_e( 'Cho phép giảm giá trên toàn cửa hàng', 'dokan' ); ?>
      </label>
    </div>
    <div
      class="show_if_needs_sw_discount <?php echo ( $is_enable_order_discount === 'yes' ) ? '' : 'hide_if_order_discount'; ?>">
      <div class="dokan-text-left dokan-form-group">
        <input type="text" id="setting_minimum_order_amount"
          value="<?php echo esc_attr( $setting_minimum_order_amount ); ?>"
          name="<?php echo esc_attr( OrderDiscount::SETTING_MINIMUM_ORDER_AMOUNT ); ?>"
          placeholder="<?php esc_attr_e( 'Minimum Order Amount', 'dokan' ); ?>" class="dokan-form-control input-md" />
      </div>
      <div class="dokan-text-left dokan-form-group">
        <input type="text" id="setting_order_percentage" value="<?php echo esc_attr( $setting_order_percentage ); ?>"
          name="<?php echo esc_attr( OrderDiscount::SETTING_ORDER_PERCENTAGE ); ?>"
          placeholder="<?php esc_attr_e( 'Percentage', 'dokan' ); ?>" class="dokan-form-control input-md" />
      </div>
    </div>
  </div>
</div>