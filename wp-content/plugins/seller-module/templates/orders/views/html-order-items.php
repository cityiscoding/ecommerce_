  ...
  <table cellpadding="0" cellspacing="0" class="woocommerce_order_items dokan-table dokan-table-strip">
    <thead>
      <tr>
        ...
        <th class="line_cost sortable" data-sort="float"><?php esc_html_e( 'Tổng', 'dokan' ); ?></th>
        ...
        <th class="line_tax tips" data-tip="
          ...
          <input type="hidden" class="order-tax-id" name="order_taxes[<?php echo $tax_id; ?>]"
...
<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
  ...
  <table class="wc-order-totals">
    <tr>
      <td><?php esc_html_e( 'Giảm giá', 'dokan' ); ?> <span class="tips"
          title="<?php esc_attr_e( 'Đây là tổng chiết khấu.Giảm giá được xác định cho mỗi mục dòng.', 'dokan' ); ?>"><span
            class="fa fa-question-circle dokan-vendor-order-page-tips"></span></span> :</td>
      <td></td>
      <td class="total">-
    ...
    <tr>
      <td><?php esc_html_e( 'Đang chuyển hàng', 'dokan' ); ?> <span class="tips"
          title="<?php esc_attr_e( 'This is the shipping and handling total costs for the order.', 'dokan' ); ?>"><span
            class="fa fa-question-circle dokan-vendor-order-page-tips"></span></span> :</td>
      <td></td>
      <td class="total">
        <?php echo wc_price( $order->get_total_shipping(), array( 'currency' => dokan_replace_func( 'get_order_currency', 'get_currency', $order ) ) ); ?>
      </td>
    </tr>
    ...
    <tr>
      <td><?php esc_html_e( 'Tổng cộng', 'dokan' ); ?>:</td>
      <td></td>
      <td class="total">
        <div class="view"><?php echo $order->get_formatted_order_total(); ?></div>
        <div class="edit" style="display: none;">
          <input type="text" class="wc_input_price" id="_order_total" name="_order_total"
            placeholder="<?php echo wc_format_localized_price( 0 ); ?>"
            value="<?php echo ( isset( $data['_order_total'][0] ) ) ? esc_attr( wc_format_localized_price( $data['_order_total'][0] ) ) : ''; ?>" />
          <div class="clear"></div>
        </div>
      </td>
    </tr>
    ...
    <tr>
      <td class="refunded-total"><?php esc_html_e( 'Hoàn lại tiền', 'dokan' ); ?>:</td>
      <td></td>
      <td class="total refunded-total">
        -<?php echo wc_price( $order->get_total_refunded(), array( 'currency' => dokan_replace_func( 'get_order_currency', 'get_currency', $order ) ) ); ?>
      </td>
    </tr>
...
<div class="wc-order-data-row wc-order-bulk-actions">
  <p class="add-items">
    ...
    <button type="button"
      class="dokan-btn dokan-btn-default refund-items"><?php esc_html_e( 'Xử lý yêu cầu hoàn lại tiền của người mua', 'dokan' ); ?></button>
    <?php endif; ?>
  </p>
  <div class="clear"></div>
</div>
...
<div class="wc-order-data-row wc-order-refund-items" style="display: none;">
  <table class="wc-order-totals dokan-table dokan-table-strip">
    ...
    <tr>
      <td><?php esc_html_e( 'Đã bổ sung hoàn trả các mặt hàng', 'dokan' ); ?>:</td>
      <td class="total"><input type="checkbox" id="restock_refunded_items" name="restock_refunded_items"
          <?php checked( apply_filters( 'dokan_restock_refunded_items', true ) ); ?> /></td>
    </tr>
    <?php endif; ?>

    <tr>
      <td><?php esc_html_e( 'Số tiền đã được hoàn lại', 'dokan' ); ?>:</td>
      <td class="total">
        -<?php echo wc_price( $order->get_total_refunded(), array( 'currency' => dokan_replace_func( 'get_order_currency', 'get_currency', $order ) ) ); ?>
      </td>
    </tr>
    <tr>
      ...
      <td class="total">
        <?php echo wc_price( $order->get_total() - $order->get_total_refunded(), array( 'currency' => dokan_replace_func( 'get_order_currency', 'get_currency', $order ) ) ); ?>
      </td>
    </tr>
    <tr>
      <td>
        <label for="refund_amount">
          <?php 
        esc_html_e( 'Nhập số tiền yêu cầu hoàn trả:', 'dokan' ); 
        echo '<br>';
        esc_html_e( '(Tổng số có thể hoàn trả)', 'dokan' ); 
        ?>
        </label>
      </td>
      <td class="total">
        <input type="number" id="refund_amount" name="refund_amount" placeholder="Là tổng số có thể hoàn trả"
    ...
    <tr>
      <td><label for="refund_reason"><?php esc_html_e( 'Thông tin hoàn trả (Paypal hoặc Ngân hàng) :', 'dokan' );
          echo '<br>';
          esc_html_e( 'Có thể bao gồm lý do', 'dokan' ); 
          ?></label>
      </td>
      <td class="total">
        <input type="text" class="text" id="refund_reason" name="refund_reason" placeholder="Trao đỗi với người mua" />
        <div class="clear"></div>
      </td>
    </tr>
  </table>
  ...
  <div class="refund-actions">
    ...
    <button type="button" class="dokan-btn dokan-btn-default do-manual-refund tips"
      data-tip="<?php esc_attr_e( 'You will need to manually issue a refund through your payment gateway after using this.', 'dokan' ); ?>"><?php printf( _x( 'Refund %s Manually', 'Refund $amount Manually', 'dokan' ), $refund_amount ); ?></button>
    ...
    <?php // translators: %s: Refund amount ?>
    <button type="button"
      class="dokan-btn dokan-btn-default <?php echo esc_attr( ( false !== $payment_gateway && $payment_gateway->can_refund_order( $order ) ) ? 'do-api-refund' : 'do-manual-refund' ); ?> tips"
      data-tip="<?php esc_attr_e( 'Admin will decide whether admin manually issue a refund or through your payment gateway via API automatically.', 'dokan' ); ?>"><?php printf( _x( 'Submit Refund Request %s', 'Submit Refund Request $amount', 'dokan' ), $refund_amount ); ?></button>

    <?php endif; ?>
    <button type="button"
      class="dokan-btn dokan-btn-default cancel-action"><?php esc_html_e( 'Hủy bỏ', 'dokan' ); ?></button>
    <div class="clear"></div>
  </div>
  <div class="clear"></div>
</div>
