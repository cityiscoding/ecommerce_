...
<div class="woocommerce_variation wc-metabox closed">
  <h3>
    <button type="button" class="remove_variation button"
      rel="<?php echo esc_attr( $variation_id ); ?>"><?php _e( 'Remove', 'dokan' ); ?></button>
  ...
  <div class="woocommerce_variable_attributes wc-metabox-content">
    <div class="data">
      <p class="form-row form-row-first upload_image">
        <a href="#" class="upload_image_button <?php if ( $_thumbnail_id > 0 ) echo 'remove'; ?>"
          rel="<?php echo esc_attr( $variation_id ); ?>"><img
            src="<?php if ( ! empty( $image ) ) echo esc_attr( $image ); else echo esc_attr( wc_placeholder_img_src() ); ?>" /><input
            type="hidden" name="upload_image_id[<?php echo $loop; ?>]" class="upload_image_id"
            value="<?php echo esc_attr( $_thumbnail_id ); ?>" /></a>
      </p>
      <?php if ( wc_product_sku_enabled() ) : ?>
      <p class="sku form-row form-row-last">
        <label><?php esc_html_e( 'SKU', 'dokan' ); ?>: <a class="tips"
            title="<?php esc_attr_e( 'Enter a SKU for this variation or leave blank to use the parent product SKU.', 'dokan' ); ?>"
            href="#"><span class="dashicons dashicons-editor-help"></span></a></label>
      ...
      <p class="form-row form-row-full options">
        <label><input type="checkbox" class="checkbox" name="variable_enabled[<?php echo $loop; ?>]"
            <?php checked( $variation->post_status, 'publish' ); ?> /> <?php _e( 'Enabled', 'dokan' ); ?></label>
        ...
        <label><input type="checkbox" class="checkbox variable_is_downloadable"
          ...
          <?php esc_html_e( 'Có thể tải xuống', 'dokan' ); ?> <a class="tips"
            title="<?php esc_attr_e( 'Enable this option if access is given to a downloadable file upon purchase of a product', 'dokan' ); ?>"
            href="#"><span class="dashicons dashicons-editor-help"></span></a></label>
        ...
        <label><input type="checkbox" class="checkbox variable_is_virtual"
          ...
          <?php esc_html_e( 'Ảo', 'dokan' ); ?> <a class="tips"
            title="<?php esc_attr_e( 'Enable this option if a product is not shipped or there is no shipping cost', 'dokan' ); ?>"
            href="#"><span class="dashicons dashicons-editor-help"></span></a></label>
        ...
        <label><input type="checkbox" class="checkbox variable_manage_stock"
          ...
          <?php esc_html_e( 'Quản lý kho?', 'dokan' ); ?> <a class="tips"
            title="<?php esc_attr_e( 'Kích hoạt tùy chọn này để kích hoạt quản lý kho.', 'dokan' ); ?>" href="#"><span
      ...
      <div class="variable_pricing">
        <p class="form-row form-row-first">
          ...
          <input type="text" size="5" name="variable_regular_price[<?php echo $loop; ?>]"
            value="<?php if ( isset( $_regular_price ) ) echo esc_attr( $_regular_price ); ?>" class="wc_input_price"
            placeholder="<?php _e( 'Variation price (required)', 'dokan' ); ?>" />
        </p>
        <p class="form-row form-row-last">
          <label><?php echo __( 'Sale Price:', 'dokan' ) . ' (' . get_woocommerce_currency_symbol() . ')'; ?> <a
              href="#" class="sale_schedule"><?php _e( 'Schedule', 'dokan' ); ?></a><a href="#"
              class="cancel_sale_schedule" style="display:none"><?php _e( 'Cancel schedule', 'dokan' ); ?></a></label>
          <input type="text" size="5" name="variable_sale_price[<?php echo $loop; ?>]"
            value="<?php if ( isset( $_sale_price ) ) echo esc_attr( $_sale_price ); ?>" class="wc_input_price" />
        </p>

        <div class="sale_price_dates_fields" style="display: none">
          <p class="form-row form-row-first">
            <label><?php _e( 'Sale start date:', 'dokan' ); ?></label>
            <input type="text" class="sale_price_dates_from" name="variable_sale_price_dates_from[<?php echo $loop; ?>]"
              value="<?php echo ! empty( $_sale_price_dates_from ) ? date_i18n( 'Y-m-d', $_sale_price_dates_from ) : ''; ?>"
              placeholder="<?php echo _x( 'From&hellip;', 'placeholder', 'dokan' ) ?> YYYY-MM-DD" maxlength="10"
          ...
          <p class="form-row form-row-last">
            ...
            <input type="text" name="variable_sale_price_dates_to[<?php echo $loop; ?>]"
              ...
              placeholder="<?php echo _x('To&hellip;', 'placeholder', 'dokan') ?> YYYY-MM-DD" maxlength="10"
              pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />
          </p>
        </div>
      </div>
      ...
      <div class="show_if_variation_manage_stock" style="display: none;">
        <p class="form-row form-row-first">
          <label><?php esc_html_e( 'Số lượng tồn kho:', 'dokan' ); ?> <a class="tips"
              title="<?php esc_attr_e( 'Enter a quantity to enable stock management at variation level, or leave blank to use the parent product\'s options.', 'dokan' ); ?>"
              href="#"><span class="dashicons dashicons-editor-help"></span></a></label>
          <input type="number" size="5" name="variable_stock[<?php echo $loop; ?>]"
            value="<?php if ( isset( $_stock ) ) echo esc_attr( $_stock ); ?>" step="any" />
        </p>
        <p class="form-row form-row-last">
      ...
      <div class="">
        <p class="form-row form-row-full">
          <label><?php esc_html_e( 'Tình trạng tồn kho', 'dokan' ); ?> <a class="tips"
              title="<?php esc_attr_e( 'Controls whether or not the product is listed as "in stock" or "out of stock" on the frontend.', 'dokan' ); ?>"
              href="#"><span class="dashicons dashicons-editor-help"></span></a></label>
      ...
      <div>
        ...
        <p class="form-row hide_if_variation_virtual form-row-first">
          <label><?php echo esc_html__( 'Weight', 'dokan' ) . ' (' . esc_html( get_option( 'woocommerce_weight_unit' ) ) . '):'; ?>
            <a class="tips"
              title="<?php esc_attr_e( 'Enter a weight for this variation or leave blank to use the parent product weight.', 'dokan' ); ?>"
              href="#"><span class="dashicons dashicons-editor-help"></span></a></label>
          <input type="text" size="5" name="variable_weight[<?php echo $loop; ?>]"
            value="<?php if ( isset( $_weight ) ) echo esc_attr( $_weight ); ?>"
            placeholder="<?php echo esc_attr( $parent_data['weight'] ); ?>" class="wc_input_decimal" />
        </p>
        <?php else : ?>
        <p>&nbsp;</p>
        <?php endif; ?>
        ...
        <p class="form-row dimensions_field hide_if_variation_virtual form-row-last">
          <label
            for="product_length"><?php echo __( 'Dimensions (L&times;W&times;H)', 'dokan' ) . ' (' . esc_html( get_option( 'woocommerce_dimension_unit' ) ) . '):'; ?></label>
          ...
          <input class="input-text wc_input_decimal" size="6" type="text" name="variable_width[<?php echo $loop; ?>]"
      ...
      <div>
        <p class="form-row hide_if_variation_virtual form-row-full">
          <label><?php _e( 'Shipping class:', 'dokan' ); ?></label> <?php
					$args = array(
        ...
        <p class="form-row form-row-full">
          ...
          <select name="variable_tax_class[<?php echo $loop; ?>]">
            <option value="parent" <?php selected( is_null( $_tax_class ), true ); ?>>
              <?php _e( 'Same as parent', 'dokan' ); ?></option>
            <?php
      ...
      <div class="show_if_variation_downloadable" style="display: none;">
        <div class="form-row form-row-full downloadable_files">
          ...
          <table class="widefat dokan-table dokan-table-strip">
            <thead>
              <div>
                <th><?php esc_html_e( 'Tên', 'dokan' ); ?> <span class="tips"
                    title="<?php esc_attr_e( 'This is the name of the download shown to the customer.', 'dokan' ); ?>"><span
                      class="dashicons dashicons-editor-help"></span></span></th>
                <th colspan="2"><?php esc_html_e( 'File URL', 'dokan' ); ?> <span class="tips"
                    title="<?php esc_attr_e( 'This is the URL or absolute path to the file which customers will get access to.', 'dokan' ); ?>"><span
                      class="dashicons dashicons-editor-help"></span></span></th>
                <th>&nbsp;</th>
              </div>
            </thead>
      ...
      <div class="show_if_variation_downloadable" style="display: none;">
        <p class="form-row form-row-first">
          <label><?php esc_html_e( 'Tải xuống giới hạn:', 'dokan' ); ?> <a class="tips"
              title="<?php esc_attr_e( 'Leave blank for unlimited re-downloads.', 'dokan' ); ?>" href="#"><span
                class="dashicons dashicons-editor-help"></span></a></label>
          <input type="number" size="5" name="variable_download_limit[<?php echo $loop; ?>]"
            ...
            placeholder="<?php _e( 'Unlimited', 'dokan' ); ?>" step="1" min="0" />
        </p>
        <p class="form-row form-row-last">
          <label><?php esc_html_e( 'Tải xuống hết hạn:', 'dokan' ); ?> <a class="tips"
              title="<?php esc_attr_e( 'Enter the number of days before a download link expires, or leave blank.', 'dokan' ); ?>"
              href="#"><span class="dashicons dashicons-editor-help"></span></a></label>
          <input type="number" size="5" name="variable_download_expiry[<?php echo $loop; ?>]"
            value="<?php if ( isset( $_download_expiry ) ) echo esc_attr( $_download_expiry ); ?>"
            placeholder="<?php _e( 'Unlimited', 'dokan' ); ?>" step="1" min="0" />
        </p>
      </div>
