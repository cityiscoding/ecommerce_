...
<p><?php esc_html_e( 'Trạng thái vận chuyển sau đây đã được thêm vào đơn đặt hàng của bạn:', 'dokan' ); ?></p>

<blockquote>
  <?php echo wpautop( wptexturize( make_clickable( $ship_info ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
</blockquote>

<p><b><?php echo $tracking_link; ?></b></p>

<p><?php esc_html_e( 'Lưu ý, đây là chi tiết đơn đặt hàng của bạn:', 'dokan' ); ?></p>
    ...
    <thead>
      <tr>
        <th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;">
          <?php esc_html_e( 'Sản phẩm', 'dokan' ); ?></th>
        <th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;">
          <?php esc_html_e( 'Số lượng', 'dokan' ); ?></th>
      </tr>
    </thead>
    <tbody>
      <?php if ( $item_qty ) : ?>
      ...
      <tr>
        <td class="td"
          style="text-align: left;vertical-align: middle;font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;color: #636363;border: 1px solid #e5e5e5;padding: 12px">
          <?php if ( $_product ) : ?>
        ...
        <td class="td"
          style="text-align: left;vertical-align: middle;font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif;color: #636363;border: 1px solid #e5e5e5;padding: 12px">
          <strong><?php echo esc_html( $item ); ?> (<?php esc_html_e( 'Số lượng', 'dokan' ); ?>)</strong>
        </td>
      </tr>
      <?php endforeach; ?>
      <?php endif; ?>
      <tr>
        <th class="td" scope="row"
          style="text-align: left;color: #636363;border: 1px solid #e5e5e5;vertical-align: middle;padding: 12px">
          <?php esc_attr_e( 'Payment Method', 'dokan' ); ?>:
        </th>
    ...
    <tfoot>
      ...
      <tr>
        <th class="td" scope="row" colspan="2" style="text-align:<?php echo esc_attr( $text_align ); ?>;">
          <?php esc_html_e( 'Ghi chú:', 'dokan' ); ?></th>
