...
<div class="dokan-lite-missing-notice dokan-alert notice">
  <div class="notice-content">
    ...
    <div class="dokan-message">
      <h3><?php esc_html_e( 'PallPall-Seller Pro đã sẵn sàng!', 'dokan' ); ?></h3>
      <?php if ( $is_dokan_installed ): ?>
      <div>
        <?php echo sprintf( __( 'You just need to activate the <strong>%s</strong> to make it functional.', 'dokan' ), 'Dokan (Lite) - Multi-vendor Marketplace plugin' ); ?>
      </div>
      <a class="dokan-btn dokan-btn-primary"
        href="<?php echo wp_nonce_url( 'plugins.php?action=activate&amp;plugin=' . $core_plugin_file . '&amp;plugin_status=all&amp;paged=1&amp;s=', 'activate-plugin_' . $core_plugin_file ); ?>"
        title="<?php esc_attr_e( 'Activate this plugin', 'dokan' ); ?>"><?php esc_html_e( 'Kích hoạt', 'dokan' ); ?></a>
      <?php else: ?>
      <div>
        <?php echo sprintf( __( "You just need to install the %sCore Plugin%s to make it functional.", "dokan" ), '<a href="https://wordpress.org/plugins/dokan-lite/">', '</a>' ); ?>
      </div>
      <button
        class="dokan-btn dokan-btn-primary install-dokan-lite"><?php esc_html_e( 'Cài đặt ngay', 'dokan' ); ?></button>
      <?php endif; ?>
    </div>

    <a href="<?php echo wp_nonce_url( 'plugins.php?action=deactivate&amp;plugin=' . $plugin_file . '&amp;plugin_status=all&amp;paged=1&amp;s=', 'deactivate-plugin_' . $plugin_file ); ?>"
      class="close-notice" title="<?php esc_attr_e( 'Dismiss this notice', 'dokan' ); ?>">
