...
<div
  class="hide_if_variable-subscription dokan-product-shipping-tax hide_if_grouped hide_if_external dokan-edit-row dokan-clearfix dokan-border-top <?php echo ! $wc_shipping_enabled ? 'woocommerce-no-shipping' : ''; ?> <?php echo ! $wc_tax_enabled ? 'woocommerce-no-tax' : ''; ?> ">
  <div class="dokan-section-heading" data-togglehandler="dokan_product_shipping_tax">
    <h2 class="hide_if_virtual"><i class="fas fa-truck" aria-hidden="true"></i> <?php echo esc_html( $tab_title ); ?>
    </h2>
    <h2 class="show_if_virtual"><i class="fas fa-truck" aria-hidden="true"></i> <?php esc_html_e( 'Thuế', 'dokan' ); ?>
    </h2>
    <p class="hide_if_virtual"><?php echo esc_html( $tab_desc ); ?></p>
    <p class="show_if_virtual"><?php esc_html_e( 'Quản lý thuế cho sản phẩm này', 'dokan' ); ?></p>
    ...
    <div class="dokan-clearfix dokan-shipping-container hide_if_virtual">
      ...
      <div class="dokan-form-group">
        <label class="dokan-checkbox-inline" for="_disable_shipping">
          ...
          <input type="checkbox" id="_disable_shipping" name="_disable_shipping" value="no"
            <?php checked( $_disable_shipping, 'no' ); ?>>
          <?php esc_html_e( 'Sản phẩm này yêu cầu vận chuyển', 'dokan' ); ?>
        </label>
      </div>
      ...
      <div class="show_if_needs_shipping dokan-form-group">
        <label class="control-label"
          for="product_shipping_class"><?php esc_html_e( 'Lớp vận chuyển', 'dokan' ); ?></label>
        ...
        <div class="dokan-additional-shipping-wrap show_if_override">
          <div class="dokan-form-group dokan-w3">
            <label class="dokan-control-label"
              for="_additional_product_price"><?php esc_html_e( 'Chi phí bổ sung', 'dokan' ); ?></label>
            <input id="_additional_product_price" value="<?php echo $_additional_price; ?>" name="_additional_price"
              placeholder="9.99" class="dokan-form-control" type="number" step="any">
          </div>

          <div class="dokan-form-group dokan-w3">
            <label class="dokan-control-label"
              for="dps_additional_qty"><?php esc_html_e( 'Giá ưu đãi mua nhiều', 'dokan' ); ?></label>
            <input id="additional_qty"
              value="<?php echo ( $_additional_qty ) ? $_additional_qty : $dps_additional_qty; ?>"
              name="_additional_qty" placeholder="1.99" class="dokan-form-control" type="number" step="any">
          </div>

          <div class="dokan-form-group dokan-w3 last-child">
            <label class="dokan-control-label"
              for="_dps_processing_time"><?php esc_html_e( 'Thời gian xử lý', 'dokan' ); ?></label>
            <select name="_dps_processing_time" id="_dps_processing_time" class="dokan-form-control">
              ...
              <option value="<?php echo $processing_key; ?>"
                <?php selected( $porduct_shipping_pt, $processing_key ); ?>><?php echo $processing_value; ?></option>
              <?php endforeach ?>
            </select>
          </div>
          <div class="dokan-clearfix"></div>
    ...
    <div class="dokan-clearfix dokan-tax-container show_if_variable show_if_simple show_if_subscription">
      <div class="dokan-tax-product-options">
        <div class="dokan-form-group content-half-part">
          <label class="dokan-control-label"
            for="_tax_status"><?php esc_html_e( 'Tình trạng thuế', 'dokan' ); ?></label>
          <div class="dokan-text-left">
            <?php
                        dokan_post_input_box(
        ...
        <div class="dokan-form-group content-half-part">
          <label class="dokan-control-label" for="_tax_class"><?php esc_html_e( 'Lớp thuế', 'dokan' ); ?></label>
          <div class="dokan-text-left">
