...
<div class="dokan-form-horizontal">
  <div class="dokan-form-group">
    ...
    <div class="dokan-w4 dokan-text-left">
      <?php dokan_post_input_box( $post->ID, '_sku', array( 'placeholder' => __( 'Số lượng sản phẩm trong kho', 'dokan' ) ) ); ?>
    </div>
  </div>

  <div class="dokan-form-group">
    <label class="dokan-w4 dokan-control-label" for=""><?php _e( 'Quản lý kho?', 'dokan' ); ?></label>
    <div class="dokan-w6 dokan-text-left">
      <?php dokan_post_input_box( $post->ID, '_manage_stock', array('label' => __( 'Bật quản lý kho ở cấp sản phẩm', 'dokan' ) ), 'checkbox' ); ?>
    </div>
  </div>
  ...
  <div class="dokan-form-group show_if_stock">
    ...
    <div class="dokan-w4 dokan-text-left">
      <input type="number" name="_stock" id="_stock" step="any" placeholder="10"
        value="<?php echo wc_stock_amount( get_post_meta( $post->ID, '_stock', true ) ); ?>">
    </div>
  </div>

  <div class="dokan-form-group hide_if_variable <?php echo ( $product_type == 'simple' ) ? 'show_if_stock' : ''; ?>">
    <label class="dokan-w4 dokan-control-label"
      for="_stock_status"><?php _e( 'Trạng thái kho hàng', 'dokan' ); ?></label>
    <div class="dokan-w4 dokan-text-left">
  ...
  <div class="dokan-form-group show_if_stock">
    <label class="dokan-w4 dokan-control-label"
      for="_backorders"><?php _e( 'Cho phép đặt hàng trước', 'dokan' ); ?></label>
    <div class="dokan-w4 dokan-text-left">
  ...
  <div class="dokan-form-group">
    ...
    <div class="dokan-w4 dokan-text-left">
      <?php dokan_post_input_box( $post->ID, '_tax_status', array( 'options' => array(
                    'taxable'   => __( 'Chịu thuế', 'dokan' ),
                    'shipping'  => __( 'Chỉ vận chuyển', 'dokan' ),
                    'none'      => _x( 'Không có', 'Tax status', 'dokan' )
                    ) ), 'select'
                ); ?>
    </div>
  </div>
