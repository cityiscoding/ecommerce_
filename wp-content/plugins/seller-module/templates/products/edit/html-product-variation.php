...
<div class="dokan-product-variation-itmes">
  ...
  <div class="actions">
    <i class="fas fa-bars sort tips"
      data-title="<?php esc_attr_e( 'Kéo thả hoặc nhấp để sắp xếp thứ tự biến thể hiển thị.', 'dokan' ); ?>"
      aria-hidden="true"></i>
    ...
    <a href="#" class="remove_variation delete"
      rel="<?php echo esc_attr( $variation_id ); ?>"><?php esc_html_e( 'Xóa', 'dokan' ); ?></a>
  </div>
    ...
    <div class="data">
      <div class="content-half-part thumbnail-checkbox-options">
        <div class="upload_image">
          <a href="#" class="upload_image_button tips <?php echo ( $_thumbnail_id > 0 ) ? 'dokan-img-remove' : ''; ?>"
            title="<?php ( $_thumbnail_id > 0 ) ? esc_attr_e( 'Xóa ảnh này', 'dokan' ) : esc_attr_e( 'Tải ảnh lên', 'dokan' ); ?>"
            rel="<?php echo esc_attr( $variation_id ); ?>">
            <img alt=" *Ấn để tải ảnh*"
              src="<?php echo ( ! empty( $image ) ? esc_attr( $image ) : esc_attr( wc_placeholder_img_src() ) ); ?>"
        ...
        <div class="dokan-form-group options">
          <label><input type="checkbox" class="" name="variable_enabled[<?php echo esc_attr( $loop ); ?>]"
              <?php checked( $variation->post_status, 'publish' ); ?> />
            <?php esc_html_e( 'Đã bật', 'dokan' ); ?></label>
          ...
          <label><input type="checkbox" class="variable_is_downloadable"
              name="variable_is_downloadable[<?php echo esc_attr( $loop ); ?>]"
              <?php checked( ! empty( $_downloadable ) ? $_downloadable : ( $sell_digital_products ? 'yes' : '' ), 'yes' ); ?> />
            <?php esc_html_e( 'Có thể tải xuống', 'dokan' ); ?> <i class="fas fa-question-circle tips"
              aria-hidden="true"
              title="<?php esc_attr_e( 'Bật tùy chọn này nếu cho phép truy cập tệp tải xuống khi mua sản phẩm.', 'dokan' ); ?>"></i></label>
          <label><input type="checkbox" class="variable_is_virtual"
              name="variable_is_virtual[<?php echo esc_attr( $loop ); ?>]"
              <?php checked( ! empty( $_virtual ) ? $_virtual : ( $sell_digital_products ? 'yes' : '' ), 'yes' ); ?> />
            <?php esc_html_e( 'Sản phẩm ảo', 'dokan' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true"
              title="<?php esc_attr_e( 'Bật tùy chọn này nếu sản phẩm không được vận chuyển hoặc không có phí vận chuyển.', 'dokan' ); ?>"></i></label>
          <?php endif; ?>
          <?php if ( 'yes' === get_option( 'woocommerce_manage_stock' ) ) : ?>
          <label><input type="checkbox" class="variable_manage_stock"
              name="variable_manage_stock[<?php echo esc_attr( $loop ); ?>]"
              <?php checked( isset( $_manage_stock ) ? $_manage_stock : '', 'yes' ); ?> />
            <?php esc_html_e( 'Quản lý kho?', 'dokan' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true"
              data-title="<?php esc_attr_e( 'Bật tùy chọn này để quản lý tồn kho theo từng biến thể.', 'dokan' ); ?>"></i></label>
          <?php endif; ?>
      ...
      <div class="content-half-part">
        ...
        <div class="sku">
          <label><?php esc_html_e( 'Số lượng tồn kho của sản phẩm này (SKU)', 'dokan' ); ?> <i
              class="fas fa-question-circle tips" aria-hidden="true"
              data-title="<?php esc_attr_e( 'Enter a SKU for this variation or leave blank to use the parent product SKU.', 'dokan' ); ?>"></i></label>
          <input type="text" class="dokan-form-control" size="5" name="variable_sku[<?php echo esc_attr( $loop ); ?>]"
        ...
        <div class="stock-status hide_if_variation_manage_stock">
          <label><?php esc_html_e( 'Tình trạng tồn kho', 'dokan' ); ?> <i class="fas fa-question-circle tips"
              aria-hidden="true"
              data-title="<?php esc_attr_e( 'Kiểm soát xem sản phẩm có được liệt kê là "trong kho" hay "hết hàng" trên nền tảng hay không.', 'dokan' ); ?>"></i></label>
      ...
      <div class="variable_pricing show_if_variable show_if_variable-subscription">
        ...
        <div class="content-half-part hide_if_variable-subscription" style="padding-right: 16px;">
          <label><?php echo __( 'Giá gốc', 'dokan' ) . ' (' . get_woocommerce_currency_symbol() . ')'; ?></label>
          <input type="text" size="5" name="variable_regular_price[<?php echo esc_attr( $loop ); ?>]"
            ...
            class="wc_input_price dokan-form-control dokan-product-regular-price-variable"
            placeholder="<?php esc_attr_e( 'Variation price (required)', 'dokan' ); ?>" />
        </div>
        <div class="content-half-part content-width-subscription">
          <label><?php echo __( 'Giá giảm còn', 'dokan' ) . ' (' . get_woocommerce_currency_symbol() . ')'; ?> <a
              href="#" class="sale_schedule"><?php esc_html_e( 'Thêm lịch trình giảm giá', 'dokan' ); ?></a><a href="#"
              class="cancel_sale_schedule"
              style="display:none"><?php esc_html_e( 'Hủy lịch trình', 'dokan' ); ?></a></label>
          <input type="text" size="5" name="variable_sale_price[<?php echo esc_attr( $loop ); ?>]"
            ...
            class="wc_input_price dokan-form-control dokan-product-sales-price-variable" />
        </div>
        <div class="dokan-form-group dokan-clearfix product-edit-container">
          <div class="dokan-product-less-price-alert dokan-hide">
            <?php esc_html_e( 'Giá sản phẩm có thể ít hơn phí nhà cung cấp!', 'dokan' ); ?>
          </div>
        </div>
        <div class="dokan-clearfix"></div>
        <div class="sale_price_dates_fields dokan-form-group" style="display: none">
          <div class="content-half-part">
            <label><?php esc_html_e( 'Ngày bắt đầu bán', 'dokan' ); ?></label>
            <input type="text" class="dokan-form-control sale_price_dates_from datepicker"
              ...
              value="<?php echo ! empty( $_sale_price_dates_from ) && is_numeric( $_sale_price_dates_from ) ? $now->setTimestamp( $_sale_price_dates_from )->format( 'Y-m-d' ) : ''; ?>"
              placeholder="<?php echo esc_attr_x( 'From&hellip;', 'placeholder', 'dokan' ); ?> YYYY-MM-DD"
              maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />
          </div>
          <div class="content-half-part">
            <label><?php esc_html_e( 'Ngày kết thúc bán hàng', 'dokan' ); ?></label>
            <input type="text" class="dokan-form-control sale_price_dates_to datepicker"
              name="variable_sale_price_dates_to[<?php echo esc_attr( $loop ); ?>]"
              value="<?php echo ! empty( $_sale_price_dates_to ) && is_numeric( $_sale_price_dates_to ) ? $now->setTimestamp( $_sale_price_dates_to )->format( 'Y-m-d' ) : ''; ?>"
              placeholder="<?php echo esc_attr_x( 'To&hellip;', 'placeholder', 'dokan' ); ?> YYYY-MM-DD" maxlength="10"
      ...
      <div class="dokan-form-group show_if_variation_manage_stock" style="display: none;">
        <div class="content-half-part">
          <label><?php esc_html_e( 'Số lượng hàng tồn kho', 'dokan' ); ?> <i class="fas fa-question-circle tips"
              aria-hidden="true"
              data-title="<?php esc_attr_e( 'Enter a quantity to enable stock management at variation level, or leave blank to use the parent product\'s options.', 'dokan' ); ?>"></i></label>
          <input type="number" class="dokan-form-control" size="5"
            name="variable_stock[<?php echo esc_attr( $loop ); ?>]"
            value="<?php echo ( isset( $_stock ) ) ? esc_attr( wc_stock_amount( $_stock ) ) : ''; ?>" step="any" />
        </div>
        <div class="content-half-part">
          <label><?php esc_html_e( 'Cho phép đặt hàng trước ?(Allow backorders?)', 'dokan' ); ?></label>
        ...
        <div class="content-half-part">
          <label
            for="variable_low_stock_amount[<?php echo esc_attr( $loop ); ?>]"><?php esc_html_e( 'Ngưỡng tồn kho thấp', 'dokan' ); ?>
            <i class="fas fa-question-circle tips" aria-hidden="true"
              data-title="<?php esc_attr_e( 'When variation stock reaches this amount you will be notified by email.', 'dokan' ); ?>"></i></label>
          <input type="number" class="dokan-form-control"
            id="variable_low_stock_amount[<?php echo esc_attr( $loop ); ?>]"
            name="variable_low_stock_amount[<?php echo esc_attr( $loop ); ?>]"
            value="<?php echo ( isset( $_low_stock_amount ) ) ? esc_attr( wc_format_localized_decimal( $_low_stock_amount ) ) : ''; ?>"
            min="0" step="1" />
        </div>
        <div class="dokan-clearfix"></div>
        <?php
      ...
      <div class="weight-dimension">
        ...
        <div class="content-half-part hide_if_variation_virtual">
          <label><?php echo __( 'Weight', 'dokan' ) . ' (' . esc_html( get_option( 'woocommerce_weight_unit' ) ) . ')'; ?>
            <i class="fas fa-question-circle tips" aria-hidden="true"
              data-title="<?php esc_attr_e( 'Enter a weight for this variation or leave blank to use the parent product weight.', 'dokan' ); ?>"></i></label>
          <input type="text" size="5" name="variable_weight[<?php echo esc_attr( $loop ); ?>]"
            value="<?php echo ( isset( $_weight ) ) ? esc_attr( $_weight ) : ''; ?>"
            placeholder="<?php echo esc_attr( $parent_data['weight'] ); ?>" class="dokan-form-control" />
        </div>
        <?php else : ?>
        <div>&nbsp;</div>
        <?php endif; ?>
        ...
        <div class="content-half-part dimensions_field hide_if_variation_virtual">
          <label
            for="product_length"><?php echo __( 'Kích thước (Chiều dài&times;Chiều rộng&times;Chiều cao)', 'dokan' ) . ' (' . esc_html( get_option( 'woocommerce_dimension_unit' ) ) . ')'; ?></label>
          <div class="dokan-form-group">
            <input id="product_length[<?php echo esc_attr( $loop ); ?>]"
              class="dokan-w3 dokan-form-control wc_input_decimal" size="6" type="text"
            ...
            <input class="dokan-w3 dokan-form-control wc_input_decimal" size="6" type="text"
            ...
            <input class="dokan-w3 dokan-form-control wc_input_decimal last" size="6" type="text"
      ...
      <div>
        ...
        <div class="dokan-form-group hide_if_variation_virtual">
          <label><?php esc_html_e( 'Lớp vận chuyển', 'dokan' ); ?></label>
          <?php
                        $args = array(
                            'taxonomy'          => 'product_shipping_class',
                            'hide_empty'        => 0,
                            'show_option_none'  => __( 'Đang phát triển...', 'dokan' ),
        ...
        <div class="dokan-form-group form-row-full">
          <label><?php esc_html_e( 'Lớp thuế', 'dokan' ); ?></label>
          <select class="dokan-form-control" name="variable_tax_class[<?php echo esc_attr( $loop ); ?>]">
            <option value="parent" <?php selected( is_null( $_tax_class ), true ); ?>>
              <?php esc_html_e( 'Tương tự sản phẩm cha', 'dokan' ); ?></option>
            <?php
      ...
      <div>
        <p class="dokan-form-group">
          <label><?php esc_html_e( 'Mô tả biến thể', 'dokan' ); ?></label>
          <textarea class="dokan-form-control" name="variable_description[<?php echo esc_attr( $loop ); ?>]" rows="3"
      ...
      <div class="show_if_variation_downloadable" style="display: none;">
        <div class="dokan-form-group downloadable_files">
          <label><?php esc_html_e( 'Tập tin có thể tải xuống', 'dokan' ); ?></label>
          <table class="dokan-table dokan-table-striped">
            <thead>
              <div>
                <th><?php esc_html_e( 'Tên', 'dokan' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true"
                    data-title="<?php esc_attr_e( 'This is the name of the download shown to the customer.', 'dokan' ); ?>"></i>
                </th>
                <th colspan="2"><?php esc_html_e( 'File URL', 'dokan' ); ?> <i class="fas fa-question-circle tips"
                    aria-hidden="true"
                    data-title="<?php esc_attr_e( 'This is the URL or absolute path to the file which customers will get access to. URLs entered here should already be encoded.', 'dokan' ); ?>"></i>
                </th>
                <th>&nbsp;</th>
              </div>
            </thead>
              ...
              <div>
                <th colspan="4">
                  <a href="#" class="dokan-btn dokan-btn-default insert-file-row" data-row="
                                    ...
                                    ?>
                                    ">
                    <?php esc_html_e( 'Thêm tập tin', 'dokan' ); ?>
                  </a>
                </th>
              </div>
            </tfoot>
      ...
      <div class="dokan-form-group show_if_variation_downloadable" style="display: none;">
        <div class="content-half-part">
          <label><?php esc_html_e( 'Tải xuống giới hạn', 'dokan' ); ?> <i class="fas fa-question-circle tips"
              aria-hidden="true"
              data-title="<?php esc_attr_e( 'Leave blank for unlimited re-downloads.', 'dokan' ); ?>"></i></label>
          <input type="text" class="dokan-form-control" name="variable_download_limit[<?php echo esc_attr( $loop ); ?>]"
            value="<?php echo ( isset( $_download_limit ) ) ? esc_attr( $_download_limit ) : ''; ?>"
            placeholder="<?php esc_attr_e( 'Unlimited', 'dokan' ); ?>" />
        </div>
        <div class="content-half-part">
          <label><?php esc_html_e( 'Tải xuống hết hạn', 'dokan' ); ?> <i class="fas fa-question-circle tips"
              aria-hidden="true"
              data-title="<?php esc_attr_e( 'Enter the number of days before a download link expires, or leave blank.', 'dokan' ); ?>"></i></label>
          <input type="text" class="dokan-form-control"
            name="variable_download_expiry[<?php echo esc_attr( $loop ); ?>]"
            value="<?php echo ( isset( $_download_expiry ) ) ? esc_attr( $_download_expiry ) : ''; ?>"
            placeholder="<?php esc_attr_e( 'Never', 'dokan' ); ?>" />
        </div>
