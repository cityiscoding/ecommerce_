<li class="product-attribute-list <?php echo esc_attr( implode( ' ', $metabox_class ) ); ?>"
  data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>">
  <div class="dokan-product-attribute-heading">
    <span><i class="fas fa-bars"
        aria-hidden="true"></i>&nbsp;&nbsp;<strong><?php echo ! empty( $attribute_label ) ? esc_html( $attribute_label ) : esc_html__( 'Tên thuộc tính', 'dokan' ); ?></strong></span>
    <a href="#" class="dokan-product-remove-attribute"><?php esc_html_e( 'Xóa', 'dokan' ); ?></a>
  ...
  <div class="dokan-product-attribute-item dokan-clearfix dokan-hide">
    <div class="content-half-part">
      <label class="form-label" for=""><?php esc_html_e( 'Tên', 'dokan' ); ?></label>
      ...
      <?php else : ?>
      <input type="text" class="attribute_name dokan-form-control dokan-product-attribute-name"
        name="attribute_names[<?php echo $i; ?>]" value="<?php echo esc_attr( $attribute['name'] ); ?>">
      <?php endif; ?>
      ...
      <label class="checkbox-item form-label">
        <input type="checkbox" <?php checked( $attribute['is_visible'], 1 ); ?>
          name="attribute_visibility[<?php echo $i; ?>]" value="1" />
        <?php esc_html_e( 'Có thể nhìn thấy trên trang sản phẩm', 'dokan' ); ?>
      </label>

      <label class="checkbox-item form-label show_if_variable show_if_variable-subscription">
        <input type="checkbox" <?php checked( $attribute['is_variation'], 1 ); ?>
          name="attribute_variation[<?php echo $i; ?>]" value="1" />
        <?php esc_html_e( 'Được sử dụng cho các phiên bản khác nhau', 'dokan' ); ?>
      </label>
    </div>

    <div class="content-half-part dokan-attribute-values">
      <label for="" class="form-label"><?php esc_html_e( 'Giá trị(s)', 'dokan' ); ?></label>
      <?php if ( $attribute['is_taxonomy'] ) : ?>
      <?php
      ...
      <div class="dokan-pre-defined-attribute-btn-group">
        <button
          class="dokan-btn dokan-btn-default plus dokan-select-all-attributes"><?php esc_html_e( 'Chọn tất cả', 'dokan' ); ?></button>
        <button
          class="dokan-btn dokan-btn-default minus dokan-select-no-attributes"><?php esc_html_e( 'Chọn không có', 'dokan' ); ?></button>

        <?php if ( dokan_get_option( 'add_new_attribute', 'dokan_selling', 'off' ) !== 'off' ) : ?>

        <button
          class="dokan-btn dokan-btn-default fr plus dokan-add-new-attribute dokan-right"><?php esc_html_e( 'Thêm mới', 'dokan' ); ?></button>

        <?php endif; ?>
      </div>
      <?php elseif ( 'text' === $attribute_taxonomy->attribute_type ) : ?>
      <?php // translators: %s:  WC DELIMITER ?>
      <select name="attribute_values[<?php echo $i; ?>][]" id="" multiple style="width:100%" class="dokan-select2"
        data-placeholder="<?php echo esc_attr( sprintf( __( 'Enter some text, or some attributes by "%s" separating values.', 'dokan' ), WC_DELIMITER ) ); ?>"
        data-tags="true" data-allow-clear="true" data-token-separators="['|']">
        <?php
      ...
      <select name="attribute_values[<?php echo $i; ?>][]" id="" multiple style="width:100%" class="dokan-select2"
        data-placeholder="<?php echo esc_attr( sprintf( __( 'Enter some text, or some attributes by "%s" separating values.', 'dokan' ), WC_DELIMITER ) ); ?>"
        data-tags="true" data-allow-clear="true" data-token-separators="['|']" data-values="[ 'Red', 'Green' ]">
