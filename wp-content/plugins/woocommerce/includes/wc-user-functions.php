...
if ( ! function_exists( 'wc_create_new_customer' ) ) {
	...
	function wc_create_new_customer( $email, $username = '', $password = '', $args = array() ) {
		if ( empty( $email ) || ! is_email( $email ) ) {
			return new WP_Error( 'registration-error-invalid-email', __( 'Vui lòng cung cấp một địa chỉ email hợp lệ.', 'woocommerce' ) );
		}

		if ( email_exists( $email ) ) {
			return new WP_Error( 'registration-error-email-exists', apply_filters( 'woocommerce_registration_error_email_exists', __( 'Một tài khoản đã được đăng ký với địa chỉ email của bạn. <a href="#" class="showlogin"> Ấn để đăng nhập.</a>', 'woocommerce' ), $email ) );
		}
		...
		if ( empty( $username ) || ! validate_username( $username ) ) {
			return new WP_Error( 'registration-error-invalid-username', __( 'Vui lòng nhập tên người dùng tài khoản hợp lệ.', 'woocommerce' ) );
		}

		if ( username_exists( $username ) ) {
			return new WP_Error( 'registration-error-username-exists', __( 'Một tài khoản đã được đăng ký với tên tên tài khoản đó. Vui lòng chọn cái khác.', 'woocommerce' ) );
		}
